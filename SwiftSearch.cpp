...
#include <algorithm>
#if defined(_MSC_VER) &&                                                       \
    _MSC_VER >= 1800 // We don't check _CPPLIB_VER here, because we want to use
                     // the <atomic> that came with the compiler, even if we use
                     // a different STL
...
namespace std {
fpos_t const std::_Fpz = {0};
}
#pragma clang diagnostic pop
#endif
EXTERN_C const GUID DECLSPEC_SELECTANY IID_IShellFolder = {
...
public:
  ...
  explicit basic_vector_based_string(const_pointer const value,
                                     size_t const n = npos)
      : base_type(value, value + static_cast<ptrdiff_t>(
                                     n == npos ? Traits::length(value) : n)) {}
  explicit basic_vector_based_string(size_type const n) : base_type(n) {}
  explicit basic_vector_based_string(size_type const n, value_type const &value)
      : base_type(n, value) {}
  explicit basic_vector_based_string(allocator_type const &ax)
      : base_type(ax) {}
  explicit basic_vector_based_string(size_type const n,
                                     allocator_type const &ax)
      : base_type(n, ax) {}
  explicit basic_vector_based_string(size_type const n, value_type const &value,
                                     allocator_type const &ax)
      : base_type(n, value, ax) {}
  ...
  void append(size_t const n, value_type const &value) {
    ...
    if (this->_End - this->_Last < static_cast<ptrdiff_t>(n)) {
      ...
      std::uninitialized_fill(this->_Last,
                              this->_Last + static_cast<ptrdiff_t>(n), value);
      this->_Last += n;
    }
#else
    this->reserve(this->size() + n);
  ...
  void append(const_pointer const value, size_type n = npos) {
    return this->append(
        value,
        value + static_cast<ptrdiff_t>(n == npos ? Traits::length(value) : n));
  }
  size_type find(value_type const &value, size_type const offset = 0) const {
    const_iterator begin = this->begin() + static_cast<difference_type>(offset),
                   end = this->end();
    size_type result =
        static_cast<size_type>(std::find(begin, end, value) - begin);
  ...
  const_pointer c_str() {
    ...
    if (n == 0 || this->capacity() <= n ||
        *(&*this->begin() + static_cast<ptrdiff_t>(n)) != value_type()) {
  ...
  iterator erase(size_t const pos, size_type const n = npos) {
    return this->erase(this->begin() + static_cast<difference_type>(pos),
                       this->begin() + static_cast<difference_type>(pos) +
                           (n == npos ? this->size() - pos : n));
  }
  iterator insert(size_t const pos, const_pointer const value,
                  size_type const n = npos) {
    return this->insert(this->begin() + static_cast<difference_type>(pos),
                        value, n);
  }
  this_type operator+(base_type const &other) const {
    this_type result;
...
#endif
  friend this_type operator+(const_pointer const left, base_type const &right) {
    size_t const nleft = Traits::length(left);
    this_type result;
  ...
  this_type &operator+=(base_type const &value) {
    if (!value.empty()) {
...
#endif
  template <class Ax2>
  friend std::basic_string<T, Traits, Ax2> &
  operator+=(std::basic_string<T, Traits, Ax2> &out, this_type const &me) {
    out.append(me.begin(), me.end());
    return out;
  }
  iterator insert(iterator const i, const_pointer const value,
                  size_type const n = npos) {
    size_type const pos = static_cast<size_type>(i - this->begin());
    this->insert(
        i, value,
        value + static_cast<ptrdiff_t>(n == npos ? Traits::length(value) : n));
    return this->begin() + static_cast<difference_type>(pos);
  }
  this_type &operator=(const_pointer const value) {
    this->clear();
    this->append(value);
    return *this;
  }
...
bool is_sorted_ex(It begin, It const end, Less less,
...
public:
  ...
  virtual pointer reallocate(pointer const p, size_t const n,
                             bool const allow_movement) = 0;
};

template <class T> class dynamic_allocator : private std::allocator<T> {
  typedef dynamic_allocator this_type;
  typedef std::allocator<T> base_type;
  typedef DynamicAllocator dynamic_allocator_type;
  dynamic_allocator_type *_alloc; /* Keep this IMMUTABLE, since otherwise we
                                     need to share state via heap allocation */
public:
  typedef typename base_type::value_type value_type;
  ...
  explicit dynamic_allocator(dynamic_allocator_type *const alloc)
      : base_type(), _alloc(alloc) {}
  template <class U>
  dynamic_allocator(dynamic_allocator<U> const &other)
      : base_type(other.base()), _alloc(other.dynamic_alloc()) {}
  ...
  pointer allocate(size_type const n, void *const p = NULL) {
    pointer r;
    if (this->_alloc) {
      r = static_cast<pointer>(this->_alloc->allocate(
          n * sizeof(value_type),
          static_cast<typename dynamic_allocator_type::pointer>(p)));
    } else {
  ...
  pointer reallocate(pointer const p, size_t const n,
                     bool const allow_movement) {
    if (this->_alloc) {
      return static_cast<pointer>(
          this->_alloc->reallocate(p, n * sizeof(value_type), allow_movement));
    } else {
      return NULL;
    }
  }
  bool operator==(this_type const &other) const {
    return static_cast<base_type const &>(*this) ==
           static_cast<base_type const &>(other);
  }
  bool operator!=(this_type const &other) const {
    return static_cast<base_type const &>(*this) !=
           static_cast<base_type const &>(other);
  }
  template <class U> this_type &operator=(dynamic_allocator<U> const &other) {
...
typedef basic_vector_based_string<TCHAR, std::char_traits<TCHAR>,
                                  dynamic_allocator<TCHAR>>
    tvstring;
} // namespace std
namespace std {
...
struct is_scalar<std::pair<T1, T2>>
...
inline void _Uninit_def_fill_n(std::pair<T1, T2> *_First, _Diff _Count,
                               _Wrap_alloc<allocator<std::pair<T1, T2>>> &,
                               _Valty *,
...
back_insert_iterator<basic_vector_based_string<T, Traits, Ax>>
copy(It begin, It end,
     back_insert_iterator<basic_vector_based_string<T, Traits, Ax>> out) {
  typedef back_insert_iterator<basic_vector_based_string<T, Traits, Ax>> Base;
  struct Derived : Base {
    using Base::container;
  };
  typename Base::container_type &container =
      *static_cast<Derived &>(out).container;
...
#define X(Class)                                                               \
  (GetProcAddress(GetModuleHandle(TEXT("win32u.dll")),                         \
                  HOOK_TYPE(Class)::static_name()))
HOOK_DEFINE_DEFAULT(HANDLE __stdcall, NtUserGetProp, (HWND hWnd, ATOM PropId),
                    X);
HOOK_DEFINE_DEFAULT(BOOL __stdcall, NtUserSetProp,
                    (HWND hWnd, ATOM PropId, HANDLE value), X);
HOOK_DEFINE_DEFAULT(LRESULT __stdcall, NtUserMessageCall,
                    (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
                     ULONG_PTR xParam, DWORD xpfnProc, BOOL bAnsi),
                    X);
HOOK_DEFINE_DEFAULT(BOOL __stdcall, NtUserRedrawWindow,
                    (HWND hWnd, CONST RECT *lprcUpdate, HRGN hrgnUpdate,
                     UINT flags),
                    X);
#undef X
...
struct atomic_flag {
  long _value;
  bool test_and_set(memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
...
public:
  ...
  atomic volatile &operator=(value_type const &value) volatile {
    this->store(value);
    return *this;
  }
  value_type
  exchange(value_type const value,
           memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
    return !!_InterlockedExchange8(const_cast<storage_type *>(&this->value),
                                   static_cast<storage_type>(value));
  }
  value_type load(memory_order const order = memory_order_seq_cst) const
      volatile {
    (void)order;
    return !!_InterlockedOr8(const_cast<storage_type *>(&this->value),
                             storage_type());
  }
  value_type store(value_type const value,
                   memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
    return !!_InterlockedExchange8(const_cast<storage_type *>(&this->value),
                                   static_cast<storage_type>(value));
...
public:
  ...
  exchange(value_type const value,
    ...
    return static_cast<value_type>(
        _InterlockedExchange(&this->value, static_cast<storage_type>(value)));
  }
  value_type load(memory_order const order = memory_order_seq_cst) const
      volatile {
    (void)order;
    return static_cast<value_type>(_InterlockedOr(
        const_cast<storage_type volatile *>(&this->value), storage_type()));
  }
  value_type store(value_type const value,
                   memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
  ...
  value_type
  fetch_add(value_type const value,
            memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
    return static_cast<value_type>(_InterlockedExchangeAdd(
        &this->value, static_cast<storage_type>(value)));
  }
  value_type
  fetch_sub(value_type const value,
            memory_order const order = memory_order_seq_cst) volatile {
...
template <> class atomic<long long> {
  ...
  storage_type value;
#ifdef _M_IX86
  static storage_type _InterlockedOr64(storage_type volatile *const result,
  ...
  static storage_type
  _InterlockedExchange64(storage_type volatile *const result,
                         storage_type const value) {
    storage_type prev;
    _ReadWriteBarrier();
    do {
  ...
  static storage_type
  _InterlockedExchangeAdd64(storage_type volatile *const result,
                            storage_type const value) {
    storage_type prev, curr;
    _ReadWriteBarrier();
    do {
...
public:
  ...
  exchange(value_type const value,
    ...
    (void)order;
    return static_cast<value_type>(
        _InterlockedExchange64(&this->value, static_cast<storage_type>(value)));
  }
  value_type load(memory_order const order = memory_order_seq_cst) const
      volatile {
    (void)order;
    return static_cast<value_type>(_InterlockedOr64(
        const_cast<storage_type volatile *>(&this->value), storage_type()));
  }
  value_type store(value_type const value,
                   memory_order const order = memory_order_seq_cst) volatile {
  ...
  fetch_add(value_type const value,
            memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
    return static_cast<value_type>(_InterlockedExchangeAdd64(
        &this->value, static_cast<storage_type>(value)));
  }
  value_type
  fetch_sub(value_type const value,
...
public:
  ...
  unique_lock() : p() {}
  explicit unique_lock(mutex_type *const mutex,
                       bool const already_locked = false)
      : p(mutex) {
    if (p && !already_locked) {
      p->lock();
    }
  }
  explicit unique_lock(mutex_type &mutex, bool const already_locked = false)
      : p(&mutex) {
    if (!already_locked) {
      p->lock();
    }
  }
  unique_lock(unique_lock const &other) : p(other.p) {
    if (p) {
      p->lock();
    }
  }
...
long global_exception_handler(struct _EXCEPTION_POINTERS *ExceptionInfo) {
  ...
  if (ExceptionInfo->ExceptionRecord->ExceptionCode ==
    ...
    atomic_namespace::unique_lock<atomic_namespace::recursive_mutex> const
        guard(global_exception_mutex);
    TCHAR buf[512];
    safe_stprintf(
        buf,
        _T("Apologies, but this program has encountered error ")
        _T("0x%lX.\r\n\r\nPLEASE tell me about this so I can try to fix it ")
        _T("for you!\r\n\r\nIf you see OK, press OK.\r\nOtherwise:\r\n- Press ")
        _T("Retry to attempt to handle the error (recommended)\r\n- Press ")
        _T("Abort to quit\r\n- Press Ignore to continue (NOT recommended)"),
        ExceptionInfo->ExceptionRecord->ExceptionCode);
    int const r =
        MessageBox(topmostWindow.m_hWnd, buf, _T("Fatal Error"),
                   MB_ICONERROR |
    ...
    result = r == IDIGNORE ? EXCEPTION_CONTINUE_EXECUTION
...
static void
append_directional(std::tvstring &str, TCHAR const sz[], size_t const cch,
                   int const ascii_mode /* -1 = decompress ASCII, 0 = no change,
                                           +1 = compress ASCII */
                   ,
  ...
  if (n + cch_in_str > str.capacity()) {
#if defined(_MSC_VER) && !defined(_CPPLIB_VER)
    dynamic_allocator<TCHAR> &alloc /* ensure correct type of allocator! */ =
        derived.allocator;
    ...
    if (TCHAR *const p = alloc.reallocate(derived._First, new_capacity,
                                          true /* TCHAR is movable */)) {
      derived._First = p;
    ...
    if (reverse) {
      if (ascii_mode < 0) {
        std::reverse_copy(
            ...
            static_cast<char const *>(static_cast<void const *>(sz)) +
                static_cast<ptrdiff_t>(cch),
            o);
      } else if (ascii_mode > 0) {
        for (size_t i = 0; i != cch; ++i) {
          static_cast<char *>(static_cast<void *>(o))[i] =
              static_cast<char>(sz[cch - 1 - i]);
        }
      } else {
      ...
      if (ascii_mode < 0) {
        std::copy(static_cast<char const *>(static_cast<void const *>(sz)),
                  static_cast<char const *>(static_cast<void const *>(sz)) +
                      static_cast<ptrdiff_t>(cch),
                  o);
      } else if (ascii_mode > 0) {
...
void CppRaiseException(unsigned long const error) {
  ...
  struct CppExceptionThrower {
    ...
    static bool assign(struct _EXCEPTION_POINTERS **to,
                       struct _EXCEPTION_POINTERS *from) {
      *to = from;
      return true;
    }
  };
  ...
  } __except (CppExceptionThrower::assign(
                  &pExPtrs, static_cast<struct _EXCEPTION_POINTERS *>(
...
LPTSTR GetAnyErrorText(DWORD errorCode, va_list *pArgList = NULL) {
  ...
  if (!FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM |
                         (pArgList == NULL ? FORMAT_MESSAGE_IGNORE_INSERTS : 0),
                     NULL, errorCode, 0, buffer,
                     sizeof(buffer) / sizeof(*buffer), pArgList)) {
    if (!FormatMessage(
            FORMAT_MESSAGE_FROM_HMODULE |
                (pArgList == NULL ? FORMAT_MESSAGE_IGNORE_INSERTS : 0),
            GetModuleHandle(_T("NTDLL.dll")), errorCode, 0, buffer,
            sizeof(buffer) / sizeof(*buffer), pArgList)) {
...
struct Wow64 {
  ...
  typedef BOOL WINAPI IsWow64Process_t(IN HANDLE hProcess,
  ...
  static bool is_wow64() {
    ...
    result = IsWow64Process && IsWow64Process(GetCurrentProcess(), &isWOW64) &&
             isWOW64;
#endif
    return result;
  }
  ...
  static void *disable() {
    ...
    if (!Wow64DisableWow64FsRedirection) {
      Wow64DisableWow64FsRedirection =
          reinterpret_cast<Wow64DisableWow64FsRedirection_t *>(GetProcAddress(
              GetKernel32(), _CRT_STRINGIZE(Wow64DisableWow64FsRedirection)));
    }
    if (Wow64DisableWow64FsRedirection &&
  ...
  static bool revert(PVOID old) {
    ...
    if (!Wow64RevertWow64FsRedirection) {
      Wow64RevertWow64FsRedirection =
          reinterpret_cast<Wow64RevertWow64FsRedirection_t *>(GetProcAddress(
              GetKernel32(), _CRT_STRINGIZE(Wow64RevertWow64FsRedirection)));
    }
    result =
        Wow64RevertWow64FsRedirection && Wow64RevertWow64FsRedirection(old);
#endif
    return result;
  }
  Wow64() {}
...
Wow64::Wow64DisableWow64FsRedirection_t *Wow64::Wow64DisableWow64FsRedirection =
    NULL;
Wow64::Wow64RevertWow64FsRedirection_t *Wow64::Wow64RevertWow64FsRedirection =
    NULL;
#endif
...
#define X(F, T)                                                                \
  identity<T>::type &F = *reinterpret_cast<identity<T>::type *const &>(        \
...
bool isdevnull(int fd) {
  ...
  return winnt::NtQueryVolumeInformationFile((HANDLE)_get_osfhandle(fd), &iosb,
                                             &fsinfo, sizeof(fsinfo), 4) == 0 &&
         fsinfo.DeviceType == 0x00000015;
}
...
struct NTFS_BOOT_SECTOR {
  ...
  unsigned int file_record_size() const {
    return this->ClustersPerFileRecordSegment >= 0
               ? this->ClustersPerFileRecordSegment * this->SectorsPerCluster *
                     this->BytesPerSector
...
struct MULTI_SECTOR_HEADER {
  ...
  bool unfixup(size_t max_size) {
    ...
    for (unsigned short i = 1; i < this->USACount; i++) {
      ...
      unsigned short *const check =
          (unsigned short *)((unsigned char *)this + offset);
      if (offset < max_size) {
        result &= *check == usa0;
        *check = usa[i];
      } else {
...
struct ATTRIBUTE_RECORD_HEADER {
  ...
  wchar_t *name() {
    return reinterpret_cast<wchar_t *>(reinterpret_cast<unsigned char *>(this) +
                                       this->NameOffset);
  }
...
struct FILE_RECORD_SEGMENT_HEADER {
  ...
  unsigned short
      SegmentNumberUpper_or_USA_or_UnknownReserved; // WARNING: This does NOT
                                                    // seem to be the actual
                                                    // "upper" segment number of
                                                    // anything! I found it to
                                                    // be 0x26e on one of my
                                                    // drives... and checkdisk
  ...
  ATTRIBUTE_RECORD_HEADER const *begin() const {
    return reinterpret_cast<ATTRIBUTE_RECORD_HEADER const *>(
        ...
        this->FirstAttributeOffset);
  }
  void *end(size_t const max_buffer_size = ~size_t()) {
    return reinterpret_cast<unsigned char *>(this) +
           (max_buffer_size < this->BytesInUse ? max_buffer_size
                                               : this->BytesInUse);
  }
...
struct REPARSE_POINT {
  ...
  void *begin() {
    return reinterpret_cast<void *>(reinterpret_cast<unsigned char *>(this) +
                                    sizeof(*this));
  }
...
void remove_path_stream_and_trailing_sep(std::tvstring &path) {
  ...
  if (!path.empty() && *(path.end() - 1) == _T('.') &&
      (path.size() == 1 || isdirsep(*(path.end() - 2)))) {
    path.erase(path.end() - 1);
  }
}
  ...
  std::tstring result =
      (SHGetDesktopFolder(&desktop) == S_OK &&
       desktop->ParseDisplayName(
           hWnd, NULL, path.empty() ? NULL : const_cast<LPWSTR>(path.c_str()),
           NULL, &shidl, &attrs) == S_OK &&
       (attrs & SFGAO_ISSLOW) == 0 &&
       desktop->GetDisplayNameOf(shidl, shgdn, &ret) == S_OK &&
       StrRetToBSTR(&ret, shidl, &bstr) == S_OK)
          ? static_cast<LPCTSTR>(bstr)
          : std::tstring(basename(path.begin(), path.end()), path.end());
  return result;
}
...
int LCIDToLocaleName_XPCompatible(LCID lcid, LPTSTR name, int name_length) {
  ...
  typedef int WINAPI LCIDToLocaleName_t(LCID Locale, LPTSTR lpName, int cchName,
                                        DWORD dwFlags);
  if (LCIDToLocaleName_t *const LCIDToLocaleName =
          reinterpret_cast<LCIDToLocaleName_t *>(
              GetProcAddress(hKernel32, _CRT_STRINGIZE(LCIDToLocaleName)))) {
    name_length = (*LCIDToLocaleName)(lcid, name, name_length, 0);
    name_length -= !!name_length;
  } else {
    ATL::CRegKey key;
    if (key.Open(HKEY_CLASSES_ROOT, TEXT("MIME\\Database\\Rfc1766"),
                 KEY_QUERY_VALUE) == 0) {
      TCHAR value_data[64 + MAX_PATH];
      TCHAR value_name[16];
      value_name[0] = _T('\0');
      safe_stprintf(value_name, _T("%04lX"), lcid);
      unsigned long value_data_length =
          sizeof(value_data) / sizeof(*value_data);
      LRESULT const result =
          key.QueryValue(value_data, value_name, &value_data_length);
      if (result == 0) {
        unsigned long i;
...
WTL::CString get_ui_locale_name() {
  return LCIDToLocaleName_XPCompatible(
...
public:
  ...
  Handle(Handle const &other) : value(other.value) {
    if (valid(this->value)) {
      CheckAndThrow(DuplicateHandle(
          GetCurrentProcess(), this->value, GetCurrentProcess(), &this->value,
...
public:
  static winnt::IO_PRIORITY_HINT query(uintptr_t const file) {
    ...
    winnt::IO_STATUS_BLOCK iosb;
    winnt::NtQueryInformationFile(reinterpret_cast<HANDLE>(file), &iosb, &old,
                                  sizeof(old), 43);
  ...
  static void set(uintptr_t const volume, winnt::IO_PRIORITY_HINT const value) {
    if (value != winnt::MaxIoPriorityTypes) {
      ...
      winnt::NTSTATUS const status =
          winnt::NtSetInformationFile(reinterpret_cast<HANDLE>(volume), &iosb,
                                      &io_priority, sizeof(io_priority), 43);
      if (status != 0 && status != 0xC0000003 /*STATUS_INVALID_INFO_CLASS*/ &&
  ...
  IoPriority(IoPriority const &other) : _volume(other._volume), _old() {
    this->_old = winnt::MaxIoPriorityTypes;
  }
  explicit IoPriority(uintptr_t const volume,
                      winnt::IO_PRIORITY_HINT const priority)
...
protected:
  ...
  _reserve(typename base_type::size_type extra_capacity) {
    typename base_type::size_type extra_reserved = 0;
#if defined(MEMORY_HEAP_HPP) && defined(_CPPLIB_VER) && 600 <= _CPPLIB_VER &&  \
    _CPPLIB_VER <= 699
    typename base_type::size_type const current_capacity =
        this->base_type::capacity();
    if (current_capacity > 0 &&
        extra_capacity > current_capacity - this->base_type::size()) {
      extra_capacity =
          this->base_type::_Grow_to(current_capacity + extra_capacity) -
          current_capacity;
      if (typename base_type::pointer const ptr =
              this->base_type::get_allocator().allocate(
                  extra_capacity, this->base_type::_Myend(), true)) {
        if (ptr == this->base_type::_Myend()) {
          this->base_type::_Myend() =
              ptr +
              static_cast<typename base_type::difference_type>(extra_capacity);
...
unsigned int get_cluster_size(void *const volume) {
  ...
  if (unsigned long const error =
          winnt::RtlNtStatusToDosError(winnt::NtQueryVolumeInformationFile(
              volume, &iosb, &info, sizeof(info), 3))) {
...
get_retrieval_pointers(TCHAR const path[], long long *const size,
  ...
  if (path) {
    HANDLE const opened =
        CreateFile(path, FILE_READ_ATTRIBUTES | SYNCHRONIZE,
                   FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
                   OPEN_EXISTING,
                   FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_NO_BUFFERING, NULL);
    unsigned long const error =
        opened != INVALID_HANDLE_VALUE ? ERROR_SUCCESS : GetLastError();
    if (!error) {
  ...
  if (handle) {
    result.resize(1 + (sizeof(RETRIEVAL_POINTERS_BUFFER) - 1) /
                          sizeof(Result::value_type));
    STARTING_VCN_INPUT_BUFFER input = {};
    BOOL success;
    for (unsigned long nr;
         !(success = DeviceIoControl(handle, FSCTL_GET_RETRIEVAL_POINTERS,
                                     &input, sizeof(input), &*result.begin(),
    ...
    result.erase(result.begin() + 1 +
                     reinterpret_cast<unsigned long const &>(*result.begin()),
                 result.end());
...
template <class T> class spin_atomic {
...
public:
  ...
  exchange(value_type value,
           memory_order const order = memory_order_seq_cst) volatile {
    (void)order;
    unique_lock_t const lock(const_cast<mutex &>(this->_mutex));
    using std::swap;
...
template <class T> struct intrusive_ptr {
  ...
  template <class U>
  intrusive_ptr(intrusive_ptr<U> const &p, bool const addref = true)
      : p(p.get()) {
    if (addref) {
      this->ref();
    }
  ...
  friend bool operator==(const_pointer const p, this_type const &me) {
    return p == me.get();
  }
  bool operator!=(const_pointer const p) const { return this->p != p; }
  friend bool operator!=(const_pointer const p, this_type const &me) {
...
public:
  ...
  void offset(long long const value) {
    ...
    this->OVERLAPPED::OffsetHigh = static_cast<unsigned long>(
        value >> (CHAR_BIT * sizeof(this->OVERLAPPED::Offset)));
  }
};

static struct negative_one {
  template <class T> operator T() const { return static_cast<T>(~T()); }
} const negative_one;
...
template <size_t V> class constant {
  template <size_t I, size_t N> struct impl;
...
template <size_t V> template <size_t I, size_t N> struct constant<V>::impl {
  static size_t multiply(size_t const m) {
    return impl<I, N / 2>::multiply(m) +
           impl<I + N / 2, N - N / 2>::multiply(m);
  }
};
template <size_t V> template <size_t I> struct constant<V>::impl<I, 1> {
  static size_t multiply(size_t const m) {
    return (V & static_cast<size_t>(static_cast<size_t>(1) << I))
...
template <class It>
typename propagate_const<typename std::iterator_traits<It>::reference,
                         typename std::iterator_traits<It>::value_type>::type *
fast_subscript(It const it, size_t const i) {
  return
#if 1
      &*(it + static_cast<ptrdiff_t>(i))
#else
      reinterpret_cast<typename propagate_const<
          typename std::iterator_traits<It>::reference,
          typename std::iterator_traits<It>::value_type>::type *>(
          reinterpret_cast<typename propagate_const<
              typename std::iterator_traits<It>::reference, unsigned char>::type
                               *>(&*it) +
          i * constant<sizeof(*it)>())
#endif
          ;
}
...
public:
  ...
  pointer tail() {
    return static_cast<unsigned char *>(this->p) +
           static_cast<ptrdiff_t>(this->n);
  }
  const_pointer tail() const {
    return static_cast<unsigned char const *>(this->p) +
           static_cast<ptrdiff_t>(this->n);
  }
  void swap(this_type &other) {
    using std::swap;
  ...
  template <class T> T *emplace_back(size_type const size = sizeof(T)) {
    size_type const old_size = this->size();
    this->resize(old_size + size);
    return new (static_cast<unsigned char *>(this->get()) +
                static_cast<difference_type>(old_size)) T;
  }
  ...
  void reserve_bytes(size_type c) {
    if (c > this->c) {
      ...
      this->p = realloc(
          this->p,
          this->n); // shrink first, to avoid copying memory beyond the block
      this->c = this->n;
...
struct Speed : std::pair<unsigned long long, clock_t> {
  Speed() {}
  Speed(first_type const &first, second_type const &second)
      : std::pair<first_type, second_type>(first, second) {}
  Speed &operator+=(Speed const &other) {
...
template <class T> struct mutable_ {
  mutable T value;
};
...
class lock_ptr
  ...
  typedef atomic_namespace::unique_lock<atomic_namespace::recursive_mutex>
      base_type;
...
public:
  ~lock_ptr() {}
  lock_ptr(T volatile *const me, bool const do_lock = true)
      : base_type(), me(const_cast<T *>(me)) {
    if (do_lock && me) {
...
template <class T>
lock_ptr<typename remove_volatile<T>::type> &
lock(T *const value,
     mutable_<lock_ptr<typename remove_volatile<T>::type>> const &holder =
         mutable_<lock_ptr<typename remove_volatile<T>::type>>()) {
  return holder.value.init(value);
}

template <class T>
lock_ptr<typename remove_volatile<T>::type> &
lock(intrusive_ptr<T> const &value,
     mutable_<lock_ptr<typename remove_volatile<T>::type>> const &holder =
         mutable_<lock_ptr<typename remove_volatile<T>::type>>()) {
...
#pragma pack(push, 1)
  ...
  public:
    ...
    file_size_type(unsigned long long const value)
        : low(static_cast<unsigned int>(value)),
          high(static_cast<unsigned short>(
              value >> (sizeof(unsigned int) * CHAR_BIT))) {}
    operator unsigned long long() const {
      return (static_cast<unsigned long long>(this->high)
              << (sizeof(unsigned int) * CHAR_BIT)) |
             this->low;
    }
    this_type &operator+=(unsigned long long const &other) {
  ...
  struct StandardInfo {
    unsigned long long created, written, accessed : 0x40 - 6, is_system : 1,
        is_directory : 1, is_sparse : 1, is_compressed : 1, is_encrypted : 1,
        is_reparse : 1;
  ...
  struct NameInfo {
    ...
    void ascii(bool const value) {
      this->_offset = static_cast<small_t<size_t>::type>(
          (this->_offset & static_cast<small_t<size_t>::type>(
                               ~static_cast<small_t<size_t>::type>(1U))) |
    ...
    void offset(small_t<size_t>::type const value) {
  ...
  struct StreamInfo : SizeInfo {
    ...
    unsigned char type_name_id : CHAR_BIT -
        2 /* zero if and only if $I30:$INDEX_ROOT or $I30:$INDEX_ALLOCATION */;
  };
  friend struct std::is_scalar<StreamInfo>;
  typedef std::codecvt<std::tstring::value_type, char, int /*std::mbstate_t*/>
      CodeCvt;
  ...
  struct ChildInfo {
    ChildInfo()
        : next_entry(negative_one), record_number(negative_one),
...
#pragma pack(push, 1)
  struct key_type_internal {
    ...
    enum {
      ...
      stream_info_bits = 13,
      index_bits =
          sizeof(unsigned int) * CHAR_BIT - (name_info_bits + stream_info_bits)
    };
    ...
    name_info_type name_info() const {
      ...
      return result == ((static_cast<name_info_type>(1) << name_info_bits) - 1)
                 ? ~name_info_type()
                 : result;
    }
    name_info_type stream_info() const {
      stream_info_type const result = this->_stream_info;
      return result == ((static_cast<stream_info_type>(1) << stream_info_bits) -
                        1)
                 ? ~stream_info_type()
                 : result;
    }
    ...
    index_type index() const {
      ...
      return result == ((static_cast<index_type>(1) << index_bits) - 1)
                 ? ~index_type()
                 : result;
    }
    void index(name_info_type const value) { this->_index = value; }
    explicit key_type_internal(frs_type const frs,
                               name_info_type const name_info,
                               stream_info_type const stream_info)
        : _frs(frs), _name_info(name_info), _stream_info(stream_info),
          _index(negative_one) {}
    bool operator==(key_type_internal const &other) const {
      return this->_frs == other._frs && this->_name_info == other._name_info &&
...
#pragma pack(pop)
  ...
  struct mapping_pair_iterator {
    ...
    struct value_type {
      value_type(vcn_type const next_vcn, lcn_type const current_lcn)
          : next_vcn(next_vcn), current_lcn(current_lcn) {}
      vcn_type next_vcn;
      lcn_type current_lcn;
    };
    explicit mapping_pair_iterator(
        ntfs::ATTRIBUTE_RECORD_HEADER const *const ah,
        size_t const max_length = ~size_t(),
        lcn_type const current_lcn = lcn_type())
        : mapping_pairs(
          ...
          ah_end(reinterpret_cast<unsigned char const *>(ah) +
                 (ah->Length < max_length
                      ? ah->Length
                      : static_cast<ptrdiff_t>(max_length))),
          j(0), value(ah->NonResident.LowestVCN, current_lcn) {}
    bool is_final() const {
    ...
    this_type &operator++() {
      ...
      {
        unsigned char v =
            static_cast<unsigned char>(lv & ((1U << (CHAR_BIT / 2)) - 1));
  ...
  Records::iterator at(size_t const frs,
    ...
    RecordsLookup::iterator const k =
        this->records_lookup.begin() + static_cast<ptrdiff_t>(frs);
    if (!~*k) {
      ptrdiff_t const j = (existing_to_revalidate ? *existing_to_revalidate
                                                  : this->records_data.end()) -
  ...
  _find(Me *const me, key_type_internal::frs_type const frs) {
    typedef
        typename propagate_const<Me, Records::value_type>::type *pointer_type;
    pointer_type result;
    if (frs < me->records_lookup.size()) {
      ...
      result =
          me->records_data.empty() ? NULL : &*(me->records_data.end() - 1) + 1;
    }
    return result;
  }

  Records::value_type *find(key_type_internal::frs_type const frs) {
    return this->_find(this, frs);
  }
  Records::value_type const *find(key_type_internal::frs_type const frs) const {
    return this->_find(this, frs);
  }
  ...
  ChildInfos::value_type const *
  childinfo(Records::value_type const *const i) const {
    return this->childinfo(i->first_child);
  }
  ChildInfos::value_type *childinfo(ChildInfo::next_entry_type const i) {
    return !~i ? NULL : fast_subscript(this->childinfos.begin(), i);
  }
  ChildInfos::value_type const *
  childinfo(ChildInfo::next_entry_type const i) const {
    return !~i ? NULL : fast_subscript(this->childinfos.begin(), i);
  }
  LinkInfos::value_type *nameinfo(LinkInfo::next_entry_type const i) {
    return !~i ? NULL : fast_subscript(this->nameinfos.begin(), i);
  }
  LinkInfos::value_type const *
  nameinfo(LinkInfo::next_entry_type const i) const {
    return !~i ? NULL : fast_subscript(this->nameinfos.begin(), i);
  }
  ...
  LinkInfos::value_type const *
  nameinfo(Records::value_type const *const i) const {
    return ~i->first_name.name.offset() ? &i->first_name : NULL;
  }
  StreamInfos::value_type *streaminfo(StreamInfo::next_entry_type const i) {
    return !~i ? NULL : fast_subscript(this->streaminfos.begin(), i);
  }
  StreamInfos::value_type const *
  streaminfo(StreamInfo::next_entry_type const i) const {
    return !~i ? NULL : fast_subscript(this->streaminfos.begin(), i);
  }
  StreamInfos::value_type *streaminfo(Records::value_type *const i) {
    assert(~i->first_stream.name.offset() ||
           (!i->first_stream.name.length && !i->first_stream.length));
    return ~i->first_stream.name.offset() ? &i->first_stream : NULL;
  }
  StreamInfos::value_type const *
  streaminfo(Records::value_type const *const i) const {
    assert(~i->first_stream.name.offset() ||
           (!i->first_stream.name.length && !i->first_stream.length));
    return ~i->first_stream.name.offset() ? &i->first_stream : NULL;
  }
...
public:
  ...
  NtfsIndex(std::tvstring value)
      : _root_path(value),
        _finished_event(CreateEvent(NULL, TRUE, FALSE, NULL)), _finished(),
        _total_names_and_streams(0), _cancelled(false), _records_so_far(0),
        _preprocessed_so_far(0), _perf_reports_circ(1 << 6),
        _perf_avg_speed(Speed()), reserved_clusters(0) {}
  ~NtfsIndex() {}
  ...
  void init() {
    ...
    HANDLE const h = CreateFile(
        path_name.c_str(), FILE_READ_DATA | FILE_READ_ATTRIBUTES | SYNCHRONIZE,
        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
    ...
    if (false) {
      IoPriority::set(reinterpret_cast<uintptr_t>(volume.value),
                      winnt::IoPriorityLow);
    }
  ...
  size_t preprocessed_so_far() const volatile {
    return this->_preprocessed_so_far.load(
        atomic_namespace::memory_order_acquire);
  }
  ...
  unsigned int get_finished() const volatile { return this->_finished.load(); }
  ...
  void preload_concurrent(unsigned long long const virtual_offset,
                          void *const buffer, size_t const size) volatile {
    ...
    assert((1U << mft_record_size_log2) == mft_record_size &&
           "MFT record size not a power of 2");
    unsigned int const mft_record_size_pow2_mod_mask = mft_record_size - 1;
    for (size_t i =
             virtual_offset &
                     mft_record_size_pow2_mod_mask /* this should only occur if
                                                      cluster size < MFT record
                                                      size */
                 ? mft_record_size - virtual_offset &
                       mft_record_size_pow2_mod_mask
                 : 0;
         i + mft_record_size <= size; i += mft_record_size) {
  ...
  void load(unsigned long long const virtual_offset, void *const buffer,
            size_t const size, unsigned long long const skipped_begin,
            unsigned long long const skipped_end) {
    unsigned int const mft_record_size = this->mft_record_size;
    unsigned int mft_record_size_log2 = 0;
    for (;;) {
      unsigned int const v = mft_record_size_log2 + 1;
    ...
    if (size % mft_record_size) {
      throw std::runtime_error(
          "Cluster size is smaller than MFT record size; split MFT records "
          "(over multiple clusters) not supported. Defragmenting your MFT may "
          "sometimes avoid this condition.");
    }
    if (skipped_begin || skipped_end) {
      this->_records_so_far.fetch_add(static_cast<unsigned int>(
          (skipped_begin + skipped_end) >> mft_record_size_log2));
    }
    ...
    for (size_t i =
         ...
         i += mft_record_size, this->_records_so_far.fetch_add(
                                   1, atomic_namespace::memory_order_acq_rel)) {
      unsigned int const frs = static_cast<unsigned int>((virtual_offset + i) >>
                                                         mft_record_size_log2);
      ntfs::FILE_RECORD_SEGMENT_HEADER *const frsh =
          reinterpret_cast<ntfs::FILE_RECORD_SEGMENT_HEADER *>(
      ...
      if (frsh->MultiSectorHeader.Magic == 'ELIF' &&
        ...
        for (ntfs::ATTRIBUTE_RECORD_HEADER const *ah = frsh->begin();
             ah < frsh_end && ah->Type != ntfs::AttributeTypeCode() &&
             ah->Type != ntfs::AttributeEnd;
             ah = ah->next()) {
          switch (ah->Type) {
          case ntfs::AttributeStandardInformation:
            if (ntfs::STANDARD_INFORMATION const *const fn =
                    static_cast<ntfs::STANDARD_INFORMATION const *>(
                        ah->Resident.GetValue())) {
              ...
              base_record->stdinfo.attributes(
                  fn->FileAttributes | ((frsh->Flags & ntfs::FRH_DIRECTORY)
          ...
          case ntfs::AttributeFileName:
            if (ntfs::FILENAME_INFORMATION const *const fn =
                    static_cast<ntfs::FILENAME_INFORMATION const *>(
                        ah->Resident.GetValue())) {
              unsigned int const frs_parent =
                  static_cast<unsigned int>(fn->ParentDirectory);
              if (fn->Flags != 0x02 /* FILE_NAME_DOS */) {
                if (LinkInfos::value_type *const si =
                        this->nameinfo(&*base_record)) {
                  size_t const link_index = this->nameinfos.size();
                  this->nameinfos.push_back(base_record->first_name);
                  base_record->first_name.next_entry =
                      static_cast<LinkInfos::value_type::next_entry_type>(
                          link_index);
                }
                LinkInfo *const info = &base_record->first_name;
                info->name.offset(
                    static_cast<unsigned int>(this->names.size()));
                info->name.length =
                    static_cast<unsigned char>(fn->FileNameLength);
                bool const ascii = is_ascii(fn->FileName, fn->FileNameLength);
                info->name.ascii(ascii);
                info->parent = frs_parent;
                append_directional(this->names, fn->FileName,
                                   fn->FileNameLength, ascii ? 1 : 0);
                if (frs_parent != frs_base) {
                  Records::iterator const parent =
                      this->at(frs_parent, &base_record);
                  ...
                  parent->first_child =
                      static_cast<ChildInfos::value_type::next_entry_type>(
                          child_index);
                }
                this->_total_names_and_streams.fetch_add(
                    base_record->stream_count,
                    atomic_namespace::memory_order_acq_rel);
          ...
          default: {
            if (ah->IsNonResident) {
              mapping_pair_iterator mpi(
                  ah, reinterpret_cast<unsigned char const *>(frsh_end) -
                          reinterpret_cast<unsigned char const *>(ah));
              for (mapping_pair_iterator::vcn_type current_vcn = mpi->next_vcn;
                   !mpi.is_final();) {
                ++mpi;
                if (mpi->current_lcn) {
                  mapping_pair_iterator::lcn_type intersect_mft_zone_begin =
                                                      this->mft_zone_start,
                                                  intersect_mft_zone_end =
                                                      this->mft_zone_end;
                  ...
                  if (intersect_mft_zone_begin < intersect_mft_zone_end) {
                    this->reserved_clusters.fetch_sub(intersect_mft_zone_end -
                                                      intersect_mft_zone_begin);
                  }
                }
                current_vcn = mpi->next_vcn;
              }
            }
            bool const is_primary_attribute =
                !(ah->IsNonResident && ah->NonResident.LowestVCN);
            if (is_primary_attribute) {
              bool const isdir =
                  (ah->Type == ntfs::AttributeBitmap ||
              ...
              unsigned char const type_name_id = static_cast<unsigned char>(
                  isdir ? 0 : ah->Type >> (CHAR_BIT / 2));
              StreamInfo *info = NULL;
              if (StreamInfos::value_type *const si =
                      this->streaminfo(&*base_record)) {
                if (isdir) // Do we want to merge this with another attribute?
                           // (e.g. AttributeIndexAllocation with
                           // AttributeIndexRoot, or secondary entry with
                           // primary entry)
                {
                  for (StreamInfos::value_type *k = si; k;
                       k = this->streaminfo(k->next_entry)) {
                    if (k->type_name_id == type_name_id &&
                        ...
                        (name_length == 0 ||
                         std::equal(
                             ah->name(),
                             ah->name() +
                                 static_cast<ptrdiff_t>(ah->NameLength),
                             this->names.begin() +
                                 static_cast<ptrdiff_t>(k->name.offset())))) {
                      info = k;
                      break;
                    }
                  }
                ...
                if (!info) {
                  ...
                  si->next_entry =
                      static_cast<small_t<size_t>::type>(stream_index);
                }
              }
              if (!info) {
                info = &base_record->first_stream;
                info->allocated =
                    0; // We have to initialize this because we add to it later
                info->length =
                    0; // We have to initialize this because we add to it later
                info->bulkiness =
                    0; // We have to initialize this because we add to it later
                info->treesize =
                    0; // We have to initialize this because we add to it later
                // We also have to initialize the name because it _identifies_
                // the attribute
                info->is_sparse = 0;
                ...
                if (isdir) {
                  ...
                  info->name.ascii(ascii);
                  append_directional(this->names, ah->name(), ah->NameLength,
                                     ascii ? 1 : 0);
                }
                ++base_record->stream_count;
                this->_total_names_and_streams.fetch_add(
              ...
              info->allocated +=
                  ah->IsNonResident
                      ? ah->NonResident.CompressionUnit
                            ? static_cast<file_size_type>(
                                  ah->NonResident.CompressedSize)
                            : static_cast<file_size_type>(
                                  is_badclus_bad /* $BadClus */
                                      ? ah->NonResident
                                            .InitializedSize /* actually this is
                                                                still wrong...
                                                                should be
                                                                looking at VCNs
                                                              */
              ...
              info->length +=
                  ah->IsNonResident
                      ? static_cast<file_size_type>(
                            is_badclus_bad /* $BadClus */
                                ? ah->NonResident
                                      .InitializedSize /* actually this is still
                                                          wrong... */
    ...
    unsigned int const records_so_far =
        this->_records_so_far.load(atomic_namespace::memory_order_acquire);
    ...
    if (finished && !this->_root_path.empty()) {
      ...
      int const nbuf = safe_stprintf(
          buf,
          _T("%s\trecords_data\twidth = %2u\tsize = %8I64u\tcapacity = ")
          _T("%8I64u\n")
          _T("%s\trecords_lookup\twidth = %2u\tsize = %8I64u\tcapacity = ")
          _T("%8I64u\n")
          _T("%s\tnames\twidth = %2u\tsize = %8I64u\tcapacity = ")
          _T("%8I64u\t%8I64u bytes wasted\n")
        ...
        PreprocessResult operator()(Records::value_type *const fr,
          ...
          if (fr) {
            ...
            for (ChildInfos::value_type *i = me->childinfo(fr);
                 i && ~i->record_number; i = me->childinfo(i->next_entry)) {
              Records::value_type *const fr2 = me->find(i->record_number);
              if (fr2 != fr) /* root directory is the only one that is a child
                                of itself */
              {
            ...
            --depth;
            std::make_heap(scratch.begin() +
                               static_cast<ptrdiff_t>(old_scratch_size),
                           scratch.end());
            unsigned long long const threshold = children_size.allocated / 100;
            // Algorithm: All files whose sizes are < X% of the current folder's
            // size are assumed to contribute to the folder's bulkiness.
            for (Scratch::iterator i = scratch.end();
                 i !=
                 scratch.begin() + static_cast<ptrdiff_t>(old_scratch_size);) {
              std::pop_heap(scratch.begin() +
                                static_cast<ptrdiff_t>(old_scratch_size),
                            i);
              --i;
              if (*i < threshold) {
            ...
            if (depth == 0) {
              children_size.allocated +=
                  static_cast<unsigned long long>(me->reserved_clusters) *
                  me->cluster_size;
            }
            result = children_size;
            struct Accumulator {
              static unsigned long long
              delta_impl(unsigned long long const value, unsigned short const i,
            ...
            StreamInfos::value_type *default_stream = NULL,
                                    *compressed_default_stream_to_merge = NULL;
            unsigned long long default_allocated_delta = 0,
                               compressed_default_allocated_delta = 0;
            for (StreamInfos::value_type *k = me->streaminfo(fr); k;
                 k = me->streaminfo(k->next_entry)) {
              bool const is_data_attribute =
                  (k->type_name_id << (CHAR_BIT / 2)) == ntfs::AttributeData;
              bool const is_default_stream =
                  is_data_attribute && !k->name.length;
              unsigned long long const
                  allocated_delta = Accumulator::delta(
                      k->is_allocated_size_accounted_for_in_main_stream
                          ? static_cast<file_size_type>(0)
                          : k->allocated,
                      name_info, total_names),
                  bulkiness_delta =
                      Accumulator::delta(k->bulkiness, name_info, total_names);
              ...
              bool const is_compression_reparse_point =
                  is_data_attribute && k->name.length &&
                  (k->name.ascii()
                       ? memcmp(reinterpret_cast<char const *>(
                                    &me->names[k->name.offset()]),
                                "WofCompressedData", 17 * sizeof(char)) == 0
                       : memcmp(reinterpret_cast<wchar_t const *>(
                                    &me->names[k->name.offset()]),
                                L"WofCompressedData",
                                17 * sizeof(wchar_t)) == 0);
              unsigned long long const length_delta = Accumulator::delta(
                  is_compression_reparse_point ? static_cast<file_size_type>(0)
                                               : k->length,
                  name_info, total_names);
              ...
              me->_preprocessed_so_far.fetch_add(
                  1, atomic_namespace::memory_order_acq_rel);
            }
            if (compressed_default_stream_to_merge && default_stream) {
              compressed_default_stream_to_merge
                  ->is_allocated_size_accounted_for_in_main_stream = 1;
              default_stream->allocated +=
                  compressed_default_stream_to_merge->allocated;
              result.allocated -= default_allocated_delta;
              result.allocated -= compressed_default_allocated_delta;
              result.allocated += Accumulator::delta(default_stream->allocated,
                                                     name_info, total_names);
            }
          }
    ...
    finished ? SetEvent(this->_finished_event)
             : ResetEvent(this->_finished_event);
  }

  void report_speed(unsigned long long const size, clock_t const tfrom,
                    clock_t const tto) {
    clock_t const duration = tto - tfrom;
    Speed const speed(size, duration);
    this->_perf_avg_speed.fetch_add(speed,
                                    atomic_namespace::memory_order_acq_rel);
    Speed const prev = this->_perf_reports_circ[this->_perf_reports_begin];
    this->_perf_reports_circ[this->_perf_reports_begin] = speed;
    this->_perf_reports_begin =
        (this->_perf_reports_begin + 1) % this->_perf_reports_circ.size();
  }
  ...
  struct file_pointers {
    ...
    key_type parent() const {
      return key_type(
          link->parent /* ... | 0 | 0 (since we want the first name of all
                          ancestors)*/
          ,
          static_cast<key_type::name_info_type>(~key_type::name_info_type()),
          static_cast<key_type::stream_info_type>(
  ...
  file_pointers get_file_pointers(key_type key) const {
    ...
    if (wait_for_finish &&
        WaitForSingleObject(this->_finished_event, 0) == WAIT_TIMEOUT) {
    ...
    if (~key.frs()) {
      ...
      unsigned short ji = 0;
      for (LinkInfos::value_type const *j = this->nameinfo(fr); j;
        ...
        if (name_info == USHRT_MAX || ji == name_info) {
          ...
          for (StreamInfos::value_type const *k = this->streaminfo(fr); k;
            ...
            if (stream_info == USHRT_MAX ? !k->type_name_id
                                         : ki == stream_info) {
              result.record = fr;
              result.link = j;
              result.stream = k;
  ...
  class ParentIterator {
    ...
    bool is_root() const { return key.frs() == 0x000000000005; }
    bool is_attribute() const {
      return ptrs.stream->type_name_id &&
             (ptrs.stream->type_name_id << (CHAR_BIT / 2)) !=
                 ntfs::AttributeData;
    }
  ...
  public:
        ...
        if (a.ascii) {
          if (b.ascii) {
            result = std::lexicographical_compare(
                static_cast<char const *>(a.first),
                static_cast<char const *>(a.first) +
                    static_cast<ptrdiff_t>(a.second),
                static_cast<char const *>(b.first),
                static_cast<char const *>(b.first) +
                    static_cast<ptrdiff_t>(b.second));
          } else {
            result = std::lexicographical_compare(
                static_cast<char const *>(a.first),
                static_cast<char const *>(a.first) +
                    static_cast<ptrdiff_t>(a.second),
                static_cast<wchar_t const *>(b.first),
                static_cast<wchar_t const *>(b.first) +
                    static_cast<ptrdiff_t>(b.second));
          }
        } else {
          if (b.ascii) {
            result = std::lexicographical_compare(
                static_cast<wchar_t const *>(a.first),
                static_cast<wchar_t const *>(a.first) +
                    static_cast<ptrdiff_t>(a.second),
                static_cast<char const *>(b.first),
                static_cast<char const *>(b.first) +
    ...
    bool operator==(this_type const &other) const {
      return this->index == other.index && this->key == other.key &&
...
#pragma clang diagnostic ignored "-Wimplicit-fallthrough"
      ...
      switch (state) {
        ...
        case 0:;
          ...
          if (!this->iteration) {
            if (is_attribute() && ptrs.stream->type_name_id <
                                      sizeof(ntfs::attribute_names) /
                                          sizeof(*ntfs::attribute_names)) {
              result.first =
                  ntfs::attribute_names[ptrs.stream->type_name_id].data;
              result.second =
                  ntfs::attribute_names[ptrs.stream->type_name_id].size;
              result.ascii = false;
              if (result.second) {
                state = 2;
                break;
...
#pragma clang diagnostic pop
  ...
  size_t get_path(key_type key, std::tvstring &result, bool const name_only,
                  unsigned int *attributes = NULL) const {
    ...
    for (ParentIterator pi(this, key);
      ...
      if (name_only ||
          !(pi->second == 1 && (pi->ascii ? *static_cast<char const *>(
                                                static_cast<void const *>(s))
                                          : *s) == _T('.'))) {
        append_directional(result, s, pi->second, pi->ascii ? -1 : 0, true);
      }
    }
    std::reverse(result.begin() + static_cast<ptrdiff_t>(old_size),
                 result.end());
  ...
  template <class F> struct Matcher {
    ...
    void operator()(key_type::frs_type const frs) {
      if (frs < me->records_lookup.size()) {
        ...
        unsigned short ji = 0;
        for (LinkInfos::value_type const *j = me->nameinfo(i); j;
    ...
    void operator()(key_type::frs_type const frs,
                    key_type::name_info_type const name_info,
                    TCHAR const stream_prefix[],
                    size_t const stream_prefix_size) {
      bool const match_paths_or_streams =
          match_paths || match_streams || match_attributes;
      bool const buffered_matching =
          stream_prefix_size || match_paths_or_streams;
      if (frs < me->records_lookup.size() &&
        ...
        for (StreamInfos::value_type const *k = me->streaminfo(fr); k;
             k = me->streaminfo(k->next_entry),
                                           new_key.stream_info(
                                               new_key.stream_info() + 1)) {
          assert(k->name.offset() <= me->names.size());
          bool const is_attribute =
              k->type_name_id &&
              (k->type_name_id << (CHAR_BIT / 2)) != ntfs::AttributeData;
          ...
          if (match_paths_or_streams) {
            if ((fr->stdinfo.attributes() & FILE_ATTRIBUTE_DIRECTORY) &&
                frs != 0x00000005) {
              path->push_back(_T('\\'));
            }
          }
          if (match_streams || match_attributes) {
            if (k->name.length) {
              path->push_back(_T(':'));
              append_directional(
                  *path, k->name.length ? &me->names[k->name.offset()] : NULL,
          ...
          if (buffered_matching) {
            name_offset =
                match_paths ? 0
                            : static_cast<unsigned int>(basename_index_in_path);
          ...
          if (frs != 0x00000005 ||
              ((depth > 0) ^
               (k->type_name_id ==
                0))) /* this is to list the root directory at the top level, but
                        its attributes at level 1 */
          {
            traverse +=
                func((buffered_matching ? path->data() : &*me->names.begin()) +
                         static_cast<ptrdiff_t>(name_offset),
        ...
        if ((frs != 0x00000005 || depth == 0) && traverse > 0) {
          ...
          for (ChildInfos::value_type const *i = me->childinfo(fr);
               i && ~i->record_number; i = me->childinfo(i->next_entry), ++ii) {
...
#pragma warning(pop)
              ...
              unsigned short const ji_target =
                  fr2->name_count - static_cast<size_t>(1) - name_index;
              unsigned short ji = 0;
              for (LinkInfos::value_type const *j = me->nameinfo(fr2); j;
                ...
                if (j->parent == frs && ji == ji_target) {
                  ...
                  name = j->name;
                  this->operator()(
                      static_cast<key_type::frs_type>(record_number), ji, NULL,
                      0);
                  if (buffered_matching) {
                    path->erase(path->end() -
                                    static_cast<ptrdiff_t>(j->name.length),
                                path->end());
                  }
                }
              }
...
struct MatchOperation {
  value_initialized<bool> is_regex, is_path_pattern, is_stream_pattern,
      requires_root_path_match;
  unsigned long long size_min;
  unsigned long long size_max;
  std::tvstring root_path_optimized_away;
  string_matcher matcher;
  std::tstring content_query;
  std::string content_query_ascii;
  MatchOperation()
      : size_min(0), size_max(static_cast<unsigned long long>(-1LL)) {}
  void init(std::tstring pattern) {
    size_t pos;
    while ((pos = pattern.find(_T("content:"))) != std::tstring::npos) {
      size_t end = pos + 8;
      if (end < pattern.size() && pattern[end] == _T('"')) {
        end++;
        size_t start = end;
        while (end < pattern.size() && pattern[end] != _T('"'))
          end++;
        content_query = pattern.substr(start, end - start);
        if (end < pattern.size())
          end++;
      } else {
        size_t start = end;
      ...
      for (size_t i = 0; i < content_query.size(); ++i) {
        content_query_ascii.push_back((char)(content_query[i] & 0xFF));
      }
    ...
    while ((pos = pattern.find(_T("size:>"))) != std::tstring::npos) {
      ...
      size_t end = pos + 6;
      while (end < pattern.size() && pattern[end] >= _T('0') &&
             pattern[end] <= _T('9')) {
        val = val * 10 + (pattern[end] - _T('0'));
        end++;
      }
      if (end < pattern.size() &&
          (pattern[end] == _T('m') || pattern[end] == _T('M'))) {
        val *= 1024 * 1024;
        end++;
        if (end < pattern.size() &&
            (pattern[end] == _T('b') || pattern[end] == _T('B')))
          end++;
      } else if (end < pattern.size() &&
                 (pattern[end] == _T('k') || pattern[end] == _T('K'))) {
        val *= 1024;
        end++;
        if (end < pattern.size() &&
            (pattern[end] == _T('b') || pattern[end] == _T('B')))
                 ...
                 (pattern[end] == _T('g') || pattern[end] == _T('G'))) {
        val *= 1024 * 1024 * 1024;
        end++;
        if (end < pattern.size() &&
            (pattern[end] == _T('b') || pattern[end] == _T('B')))
    ...
    while (!pattern.empty() && *pattern.rbegin() == _T(' '))
      pattern.erase(pattern.length() - 1);
    while (!pattern.empty() && *pattern.begin() == _T(' '))
    ...
    is_path_pattern =
        is_regex || ~pattern.find(_T('\\')) || ~pattern.find(_T("**"));
    is_stream_pattern = is_regex || ~pattern.find(_T(':'));
    requires_root_path_match =
        is_path_pattern && !is_regex && pattern.size() >= 2 &&
        *(pattern.begin() + 0) != _T('*') &&
        *(pattern.begin() + 0) != _T('?') &&
        *(pattern.begin() + 1) != _T('*') && *(pattern.begin() + 1) != _T('?');
    if (requires_root_path_match) {
      root_path_optimized_away.insert(
          root_path_optimized_away.end(), pattern.begin(),
          std::find(pattern.begin(), pattern.end(), _T('\\')));
      pattern.erase(pattern.begin(),
                    pattern.begin() + static_cast<ptrdiff_t>(
                                          root_path_optimized_away.size()));
    }
    if (!is_path_pattern && !~pattern.find(_T('*')) &&
        !~pattern.find(_T('?'))) {
    ...
    string_matcher(is_regex          ? string_matcher::pattern_regex
                   : is_path_pattern ? string_matcher::pattern_globstar
    ...
    return !requires_root_path_match ||
           (root_path.size() >= root_path_optimized_away.size() &&
            std::equal(root_path.begin(),
                       root_path.begin() + static_cast<ptrdiff_t>(
                                               root_path_optimized_away.size()),
...
#ifdef _XMEMORY_
#define X(...)                                                                 \
  template <> struct is_scalar<__VA_ARGS__> : is_pod<__VA_ARGS__> {}
...
public:
  HRESULT hr;
  OleInit(bool initialize = true)
      : hr(initialize ? OleInitialize(NULL) : S_FALSE) {}
  ~OleInit() {
    if (this->hr == S_OK) {
      OleUninitialize();
    }
...
class CDisableListViewUnnecessaryMessages
    : hook_detail::thread_hook_swap<HOOK_TYPE(NtUserMessageCall)> {
  ...
  LRESULT operator()(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
    ...
    LRESULT result;
    if (msg == LVM_GETACCVERSION && this->prev_result_LVM_GETACCVERSION.valid &&
        this->prev_result_LVM_GETACCVERSION.hwnd == hWnd) {
      result =
          msg == LVM_GETACCVERSION && this->prev_result_LVM_GETACCVERSION.valid
...
class CSetRedraw {
  static TCHAR const *key() {
    return _T("Redraw.{A303353B-C8AA-40FA-8518-C4E02B30033A}");
  }
  struct Hooked {
    ...
    struct : hook_detail::thread_hook_swap<HOOK_TYPE(NtUserRedrawWindow)> {
      BOOL operator()(HWND hWnd, CONST RECT *lprcUpdate, HRGN hrgnUpdate,
                      UINT flags) override {
        BOOL result;
        hook_type *const self = CONTAINING_RECORD(
            this, hook_type, HOOK_CONCAT(hook_, NtUserRedrawWindow));
...
class RefCountedCString
    : public WTL::CString // ref-counted in order to ensure that copying doesn't
                          // move the buffer (e.g. in case a containing vector
                          // resizes)
{
...
public:
  ...
  operator LPTSTR() { return this->c_str(); }
  friend std::basic_ostream<TCHAR> &operator<<(
      std::basic_ostream<TCHAR> &ss,
      this_type &me); // Do NOT implement this. Turns out DDK's implementation
                      // doesn't handle embedded null characters correctly. Just
                      // use a basic_string directly instead.
};
...
public:
  ...
  RefCountedCString &operator()(unsigned short const id) {
    ...
    if (this->strings[id].IsEmpty()) {
      ...
      bool success =
          mui_module && (SwapModuleResource(mui_module), !!str.LoadString(id));
      if (!success) {
        str.LoadString(id);
      }
    }
...
public:
  ...
  struct ClientDC : WTL::CClientDC {
    ...
    explicit ClientDC(HWND const wnd) : WTL::CClientDC(wnd), old_font() {
      this->old_font =
          this->SelectFont(static_cast<ATL::CWindow>(wnd).GetFont());
    }
    int GetTextWidth(String const &string) const {
      WTL::CSize sizeRect;
      this->GetTextExtent(string.data(), static_cast<int>(string.size()),
                          &sizeRect);
      return sizeRect.cx;
    }
  };
  ListViewAdapter(WTL::CListViewCtrl &me,
      ...
      : me(&me), cached_column_header_widths(&cached_column_header_widths),
  ...
  std::pair<int, int> GetVisibleItems() const {
    ...
    if (me->GetClientRect(&rc)) {
      ...
      for (int i = result.first; i < result.second; ++i) {
        if (me->GetItemRect(i, &rcitem, LVIR_BOUNDS) &&
  ...
  ImageList GetImageList() {
    ...
    return ImageList(me->GetImageList(view == LV_VIEW_DETAILS ||
                                              view == LV_VIEW_LIST ||
                                              view == LV_VIEW_SMALLICON
                                          ? LVSIL_SMALL
                                          : LVSIL_NORMAL));
  }
  bool SetColumnWidth(int const column,
                      int const width /* -1 = autosize, -2 = use header */) {
  ...
  if (m && n) {
    {
      ...
      for (int i = 0; i < static_cast<int>(m); ++i) {
        ...
        if (list->cached_column_header_widths->find(text) ==
          ...
          if (itemp1 < 0) {
            itemp1 = static_cast<int>(
                list->InsertColumn(static_cast<int>(m), empty));
          }
          if (itemp2 < 0) {
            itemp2 = static_cast<int>(
                list->InsertColumn(static_cast<int>(m), empty));
          }
          int cx = -1;
          if (text.empty()) {
            ...
            if (list->SetColumnWidth(static_cast<int>(itemp2),
                                     -2 /*AUTOSIZE_USEHEADER*/)) {
    ...
    std::vector<std::pair<std::pair<int /* priority */, int /* width */>,
                          int /* column */>>
        sizes;
    ...
    std::vector<int> column_widths(m, 0);
    {
      ...
      visible.second = visible.second + feather <= static_cast<ptrdiff_t>(n)
                           ? visible.second + feather
                           : static_cast<int>(n);
      int cximg = 0;
      if (ListViewAdapter::ImageList imagelist = list->GetImageList()) {
        int cyimg;
        if (!(imagelist->GetImageCount() > 0 &&
              imagelist->GetSize(visible.first, cximg, cyimg))) {
          cximg = 0;
        }
      }
      ...
      for (size_t j = 0; j != m; ++j) {
        {
          ...
          if (list->GetColumn(static_cast<int>(j), col)) {
            ...
            ListViewAdapter::CachedColumnHeaderWidths::const_iterator const
                found = list->cached_column_header_widths->find(text);
            size_t ibreak =
                std::find_first_of(text.begin(), text.end(), &breaks[0],
                                   &breaks[sizeof(breaks) / sizeof(*breaks)]) -
                text.begin();
            if (ibreak < text.size()) {
              text.erase(text.begin() + static_cast<ptrdiff_t>(ibreak),
                         text.end());
              for (int k = 0; k != 3; ++k) {
                text.push_back('.');
              }
          ...
          sizes.push_back(std::make_pair(std::make_pair(SHRT_MAX, cxheader),
                                         static_cast<int>(j)));
        }
        for (size_t i = visible.first >= 0 ? static_cast<size_t>(visible.first)
                                           : 0;
             i !=
             (visible.second >= 0 ? static_cast<size_t>(visible.second) : 0);
             ++i) {
          int cx = 0;
          if (ListViewAdapter::String const *const text = list->GetItemText(
                  static_cast<long>(i), static_cast<long>(j))) {
            cx = (j == 0 ? cximg : 0) + dc.GetTextWidth(*text) +
                 column_slacks[static_cast<size_t>(j)];
          }
          sizes.push_back(
              std::make_pair(std::make_pair(0, cx), static_cast<int>(j)));
        }
      }
    }
...
template <class T> class TempSwap {
...
public:
  ...
  explicit TempSwap(T &target, T const &new_value)
      : target(&target), old_value(new_value) {
    using std::swap;
    swap(*this->target, this->old_value);
  }
...
#pragma warning(suppress : 4555)
  ...
  void OnDestroy() { setTopmostWindow.reset(); }
  ...
  BOOL OnInitDialog(CWindow /*wndFocus*/, LPARAM /*lInitParam*/) {
    ...
    // reinterpret_cast<LONG_PTR>(GetSysColorBrush(COLOR_3DFACE)));
    ...
    this->lastProgressText.assign(static_cast<LPCTSTR>(bstr),
                                  bstr ? bstr.Length() : 0);
...
#pragma warning(suppress : 4555)
  ...
  static BOOL EnableWindowRecursive(HWND hWnd, BOOL enable,
                                    BOOL includeSelf = true) {
    struct Callback {
      static BOOL CALLBACK EnableWindowRecursiveEnumProc(HWND hWnd,
                                                         LPARAM lParam) {
  ...
  unsigned long WaitMessageLoop(uintptr_t const handles[],
                                size_t const nhandles) {
    for (;;) {
      unsigned long result;
      if (this->windowCreated) {
        result = MsgWaitForMultipleObjectsEx(
            static_cast<unsigned int>(nhandles),
            reinterpret_cast<HANDLE const *>(handles), UPDATE_INTERVAL,
            QS_ALLINPUT, MWMO_INPUTAVAILABLE);
      } else {
        result =
            WaitForMultipleObjectsEx(static_cast<unsigned int>(nhandles),
                                     reinterpret_cast<HANDLE const *>(handles),
                                     FALSE, UPDATE_INTERVAL, FALSE);
      }
      if (result < WAIT_OBJECT_0 + nhandles || result == WAIT_TIMEOUT) {
        return result;
      } else if (result ==
                 WAIT_OBJECT_0 + static_cast<unsigned int>(nhandles)) {
  ...
  DWORD GetMinDelay() const { return IsDebuggerPresent() ? 0 : 750; }
...
public:
  ...
  CProgressDialog(ATL::CWindow parent)
      : Base(true, !!(parent.GetExStyle() & WS_EX_LAYOUTRTL)), canceled(false),
        invalidated(false), creationTime(GetTickCount()), lastUpdateTime(0),
        parent(parent), windowCreated(false), windowCreateAttempted(false),
        windowShowAttempted(false), lastProgress(0), lastProgressTotal(1) {}
  ...
  void refresh_marquee() {
    ...
    this->progressBar.ModifyStyle(marquee ? 0 : PBS_MARQUEE,
                                  marquee ? PBS_MARQUEE : 0, 0);
  ...
  bool HasUserCancelled(unsigned long const now = GetTickCount()) {
    ...
    if (abs(static_cast<int>(now - this->creationTime)) >=
        static_cast<int>(this->GetMinDelay())) {
      this->ForceShow();
    }
  ...
  void SetProgress(long long current, long long total) {
    if (total > INT_MAX) {
      current = static_cast<long long>((static_cast<double>(current) / total) *
                                       INT_MAX);
      total = INT_MAX;
    }
    this->invalidated |=
        this->lastProgress != current || this->lastProgressTotal != total;
    bool const marquee_invalidated =
        this->invalidated && ((this->lastProgressTotal == 0) ^ (total == 0));
  ...
  void SetProgressText(std::tvstring const &text) {
    this->invalidated |=
        this->lastProgressText.size() != text.size() ||
        !std::equal(this->lastProgressText.begin(),
                    this->lastProgressText.end(), text.begin());
...
protected:
  struct Task {
    ...
    Task() : file(), buffer(), cb() {}
    explicit Task(void *const buffer, unsigned long const cb,
                  intrusive_ptr<Overlapped> const &overlapped)
        : buffer(buffer), cb(cb), overlapped(overlapped) {}
  };
  struct WorkerThread {
    Handle handle;
    ~WorkerThread() { WaitForSingleObject(this->handle, INFINITE); }
  };
...
#pragma omp atomic
  ...
  virtual unsigned int worker() volatile { return this->worker(INFINITE); }
  virtual unsigned int worker(unsigned long const timeout) volatile {
    ...
    Overlapped *p;
    try {
      for (unsigned long nr; GetQueuedCompletionStatus(
               this->_handle, &nr, &key, &overlapped_ptr, timeout);) {
        ...
        if (overlapped.get()) {
          ...
          Task task;
          {
            ...
            size_t &pending_scan_offset =
                const_cast<size_t &>(this->_pending_scan_offset);
            ...
            for (size_t o = 0; o != pending.size(); ++o) {
              if (pending_scan_offset == 0 ||
                  pending_scan_offset > pending.size()) {
                pending_scan_offset = pending.size();
              }
              --pending_scan_offset;
              size_t const i = pending_scan_offset;
              winnt::IO_PRIORITY_HINT const curr_priority = IoPriority::query(
                  reinterpret_cast<uintptr_t>(pending[i].file));
              if (found_priority == winnt::MaxIoPriorityTypes ||
                  curr_priority > found_priority) {
                found = i;
  ...
  void enqueue(Task &task) volatile {
    ...
    if (!task.cb ||
        (!this->_terminated.load(atomic_namespace::memory_order_acquire) &&
         ((attempted_reading_file = true),
          ReadFile(task.file, task.buffer, task.cb, NULL,
                   task.overlapped.get())))) {
  ...
  void cancel_thread_ios() volatile // Requests cancellation of all I/Os
                                    // initiated by the current thread
  {
    lock_ptr<this_type> const me_lock(this);
    Pending &pending = const_cast<Pending &>(this->_pending);
...
public:
  ...
  IoCompletionPort()
      : _handle(CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, NULL, 0)),
        _initialized(false), _terminated(false), _pending_scan_offset() {}
    ...
    if (this->_threads.empty()) {
      ...
      for (size_t i = nthreads; i != 0 && ((void)--i, true);) {
        ...
        Handle(reinterpret_cast<HANDLE>(
                   _beginthreadex(NULL, 0, iocp_worker, this, 0, &id)))
  ...
  void post(unsigned long cb, uintptr_t const key,
            intrusive_ptr<Overlapped> overlapped) volatile {
    CheckAndThrow(this->try_post(cb, key, overlapped));
    overlapped.detach();
  }
  bool try_post(unsigned long cb, uintptr_t const key,
                intrusive_ptr<Overlapped> &overlapped) volatile {
    this->ensure_initialized();
    if (!(cb == 0 && key == 0 && !overlapped) &&
        this->_terminated.load(atomic_namespace::memory_order_acquire)) {
      CppRaiseException(ERROR_CANCELLED);
    }
    return !!PostQueuedCompletionStatus(this->_handle, cb, key,
                                        overlapped.get());
  }
  void read_file(HANDLE const file, void *const buffer, unsigned long const cb,
...
class OleIoCompletionPort : public IoCompletionPort {
  virtual unsigned int worker() volatile override {
    ...
    } __except (global_exception_handler(
        static_cast<struct _EXCEPTION_POINTERS *>(GetExceptionInformation()))) {
...
protected:
  struct RetPtr {
    ...
    RetPtr(unsigned long long const vcn, unsigned long long const cluster_count,
           unsigned long long const lcn)
        : vcn(vcn), cluster_count(cluster_count), lcn(lcn), skip_begin(0),
          skip_end(0) {}
    RetPtr(RetPtr const &other)
        : vcn(other.vcn), cluster_count(other.cluster_count), lcn(other.lcn),
          skip_begin(
              other.skip_begin.load(atomic_namespace::memory_order_relaxed)),
          skip_end(
              other.skip_end.load(atomic_namespace::memory_order_relaxed)) {}
    RetPtr &operator=(RetPtr const &other) {
      this->vcn = other.vcn;
      this->cluster_count = other.cluster_count;
      this->lcn = other.lcn;
      this->skip_begin.store(
          other.skip_begin.load(atomic_namespace::memory_order_relaxed));
      this->skip_end.store(
          other.skip_end.load(atomic_namespace::memory_order_relaxed));
      return *this;
  ...
  atomic_namespace::atomic<RetPtrs::size_type> jbitmap, nbitmap_chunks_left,
      jdata;
...
public:
  ...
  OverlappedNtfsMftReadPayload(IoCompletionPort volatile &iocp,
                               intrusive_ptr<NtfsIndex volatile> p,
                               Handle const &closing_event)
      : Overlapped(), iocp(&iocp), closing_event(closing_event), cluster_size(),
        read_block_size(1 << 20), jbitmap(0), nbitmap_chunks_left(0), jdata(0),
        valid_records(0) {
    using std::swap;
    swap(p, this->p);
  }
...
class OverlappedNtfsMftReadPayload::ReadOperation : public Overlapped {
  ...
  static void *operator new(size_t n) {
    ...
    if (true) {
      {
        atomic_namespace::unique_lock<atomic_namespace::recursive_mutex> guard(
            recycled_mutex);
        size_t ifound = recycled.size();
        for (size_t i = 0; i != recycled.size(); ++i) {
          if (recycled[i].first >= n &&
              (ifound >= recycled.size() ||
               recycled[i].first <= recycled[ifound].first)) {
            ifound = i;
          }
        }
...
public:
  ...
  explicit ReadOperation(
      intrusive_ptr<OverlappedNtfsMftReadPayload volatile> const &q,
      bool const is_bitmap)
      : Overlapped(), _voffset(), _skipped_begin(), _skipped_end(),
        _time(clock()), _is_bitmap(is_bitmap), q(q) {}
  ...
  void skipped_begin(unsigned long long const value) {
    this->_skipped_begin = value;
  }
  unsigned long long skipped_end() { return this->_skipped_end; }
  void skipped_end(unsigned long long const value) {
    this->_skipped_end = value;
  }
  clock_t time() { return this->_time; }
  void time(clock_t const value) { this->_time = value; }
  int operator()(size_t const size, uintptr_t const /*key*/) {
    OverlappedNtfsMftReadPayload *const q =
        const_cast<OverlappedNtfsMftReadPayload *>(
            static_cast<OverlappedNtfsMftReadPayload volatile *>(
                this->q.get()));
    if (!q->p->cancelled()) {
      this->q->queue_next();
      void *const buffer = this + 1;
      if (this->_is_bitmap) {
        size_t const records_per_bitmap_word =
            sizeof(*q->mft_bitmap.begin()) * CHAR_BIT;
        if (this->voffset() * CHAR_BIT <= q->p->mft_capacity) {
          static unsigned char const popcount[] = {0, 1, 1, 2, 1, 2, 2, 3,
                                                   1, 2, 2, 3, 2, 3, 3, 4};
          unsigned int nrecords = 0;
          size_t n = size;
          if (this->voffset() + n >= q->p->mft_capacity / CHAR_BIT) {
            n = static_cast<size_t>(q->p->mft_capacity / CHAR_BIT -
                                    this->voffset());
          }
          for (size_t i = 0; i < n; ++i) {
            unsigned char const v = static_cast<unsigned char const *>(
                                    buffer)[i],
                                vlow = static_cast<unsigned char>(
                                    v >> (CHAR_BIT / 2)),
                                vhigh = static_cast<unsigned char>(
                                    v ^ (vlow << (CHAR_BIT / 2)));
            nrecords += popcount[vlow];
            nrecords += popcount[vhigh];
          }
          std::copy(static_cast<unsigned char const *>(buffer),
                    static_cast<unsigned char const *>(buffer) +
                        static_cast<ptrdiff_t>(n),
                    q->mft_bitmap.begin() +
                        static_cast<ptrdiff_t>(this->voffset()));
          q->valid_records.fetch_add(nrecords,
                                     atomic_namespace::memory_order_acq_rel);
        }
        if (q->nbitmap_chunks_left.fetch_sub(
                1, atomic_namespace::memory_order_acq_rel) == 1) {
          unsigned int const valid_records = q->valid_records.exchange(
              0 /* make sure this doesn't happen twice */,
              atomic_namespace::memory_order_acq_rel);
          lock(q->p)->reserve(valid_records);

          // Now, go remove records from the queue that we know are invalid...
          for (RetPtrs::iterator i = q->data_ret_ptrs.begin();
               i != q->data_ret_ptrs.end(); ++i) {
            size_t const irecord = static_cast<size_t>(
                             i->vcn * q->cluster_size / q->p->mft_record_size),
                         nrecords = static_cast<size_t>(i->cluster_count *
                                                        q->cluster_size /
                                                        q->p->mft_record_size);
            size_t skip_records_begin, skip_records_end;
            // TODO: We're doing a bitmap search bit-by-bit here, which is
            // slow... maybe improve it at some point... but OTOH it's still
            // much faster than I/O anyway, so whatever...
            for (skip_records_begin = 0; skip_records_begin != nrecords;
                 ++skip_records_begin) {
              size_t const j = irecord + skip_records_begin,
            ...
            for (skip_records_end = 0;
                 skip_records_end != nrecords - skip_records_begin;
                 ++skip_records_end) {
              size_t const j = irecord + nrecords - 1 - skip_records_end,
                           j1 = j / records_per_bitmap_word,
                           j2 = j % records_per_bitmap_word;
              if (q->mft_bitmap[j1] & (1 << j2)) {
                break;
            ...
            size_t skip_clusters_begin = static_cast<size_t>(
                       static_cast<unsigned long long>(skip_records_begin) *
                       q->p->mft_record_size / q->cluster_size),
                   skip_clusters_end = static_cast<size_t>(
                       static_cast<unsigned long long>(skip_records_end) *
                       q->p->mft_record_size / q->cluster_size);
            if (skip_clusters_begin + skip_clusters_end > i->cluster_count) {
              throw std::logic_error(
                  "we should never be skipping more clusters than there are");
            }
            i->skip_begin.store(skip_clusters_begin,
                                atomic_namespace::memory_order_release);
            i->skip_end.store(skip_clusters_end,
                              atomic_namespace::memory_order_release);
          }
        }
      } else {
        ...
        lock(q->p)->load(virtual_offset, buffer, size, this->skipped_begin(),
                         this->skipped_end());
      }
      {
...
atomic_namespace::recursive_mutex
    OverlappedNtfsMftReadPayload::ReadOperation::recycled_mutex;
std::vector<std::pair<size_t, void *>>
    OverlappedNtfsMftReadPayload::ReadOperation::recycled;
...
void OverlappedNtfsMftReadPayload::queue_next() volatile {
  ...
  bool handled = false;
  if (!handled) {
    size_t const jbitmap =
        this->jbitmap.fetch_add(1, atomic_namespace::memory_order_acq_rel);
    if (jbitmap < me->bitmap_ret_ptrs.size()) {
      handled = true;
      RetPtrs::const_iterator const j =
          me->bitmap_ret_ptrs.begin() + static_cast<ptrdiff_t>(jbitmap);
      unsigned long long const skip_begin = j->skip_begin.load(
                                   atomic_namespace::memory_order_acquire),
                               skip_end = j->skip_end.load(
                                   atomic_namespace::memory_order_acquire);
      unsigned int const cb = static_cast<unsigned int>(
          (j->cluster_count - (skip_begin + skip_end)) * me->cluster_size);
      intrusive_ptr<ReadOperation> p(new (cb) ReadOperation(this, true));
      p->offset((j->lcn + skip_begin) *
                static_cast<long long>(me->cluster_size));
  ...
  if (!handled) {
    size_t const jdata =
        this->jdata.fetch_add(1, atomic_namespace::memory_order_acq_rel);
    if (jdata < me->data_ret_ptrs.size()) {
      handled = true;
      RetPtrs::const_iterator const j =
          me->data_ret_ptrs.begin() + static_cast<ptrdiff_t>(jdata);
      unsigned long long const skip_begin = j->skip_begin.load(
                                   atomic_namespace::memory_order_acquire),
                               skip_end = j->skip_end.load(
                                   atomic_namespace::memory_order_acquire);
...
int OverlappedNtfsMftReadPayload::operator()(size_t const /*size*/,
                                             uintptr_t const key) {
  int result = -1;
  ...
  try {
    ...
    if (void *const volume = p->volume()) {
      ...
      CheckAndThrow(DeviceIoControl(volume, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0,
      ...
      p->mft_capacity = static_cast<unsigned int>(
          info.MftValidDataLength.QuadPart / info.BytesPerFileRecordSegment);
      p->mft_zone_start = info.MftZoneStart.QuadPart;
      p->mft_zone_end = info.MftZoneEnd.QuadPart;
      p->mft_zone_end = p->mft_zone_start; // This line suppresses MFT zone
                                           // inclusion in the "size on disk"
      p->reserved_clusters.store(info.TotalReserved.QuadPart + p->mft_zone_end -
                                 p->mft_zone_start);
      ...
      typedef std::vector<std::pair<unsigned long long, long long>> RP;
      {
        {
          long long llsize = 0;
          RP const ret_ptrs = get_retrieval_pointers(
              (p->root_path() + std::tvstring(_T("$MFT::$BITMAP"))).c_str(),
              &llsize, info.MftStartLcn.QuadPart, this->p->mft_record_size);
          unsigned long long prev_vcn = 0;
          for (RP::const_iterator i = ret_ptrs.begin(); i != ret_ptrs.end();
               ++i) {
            long long const clusters_left =
                static_cast<long long>(std::max(i->first, prev_vcn) - prev_vcn);
            unsigned long long n;
            for (long long m = 0; m < clusters_left;
                 m += static_cast<long long>(n)) {
              n = std::min(
                  i->first - prev_vcn,
                  1 + (static_cast<unsigned long long>(this->read_block_size) -
                       1) /
                          this->cluster_size);
              this->bitmap_ret_ptrs.push_back(
                  RetPtrs::value_type(prev_vcn, n, i->second + m));
              prev_vcn += n;
            }
          }
          this->mft_bitmap
              .resize(static_cast<size_t>(llsize),
                      static_cast<Bitmap::value_type>(~Bitmap::value_type()) /* default should be to read unused slots too */);
          this->nbitmap_chunks_left.store(
              this->bitmap_ret_ptrs.size(),
              atomic_namespace::memory_order_release);
        }
        {
          long long llsize = 0;
          RP const ret_ptrs = get_retrieval_pointers(
              (p->root_path() + std::tvstring(_T("$MFT::$DATA"))).c_str(),
              &llsize, info.MftStartLcn.QuadPart, p->mft_record_size);
          if (ret_ptrs.empty()) {
            CppRaiseException(ERROR_UNRECOGNIZED_VOLUME);
          }
          unsigned long long prev_vcn = 0;
      ...
      {
        set_finished.p
            .reset(); // make sure we don't set this as finished anymore
...
std::vector<std::tvstring> get_volume_path_names() {
  ...
  do {
    prev = buf.size();
    buf.resize(std::max(static_cast<size_t>(GetLogicalDriveStrings(
                            static_cast<unsigned long>(buf.size()),
                            buf.empty() ? NULL : &*buf.begin())),
                        buf.size()));
  } while (prev < buf.size());
  for (size_t i = 0, n;
       n = std::char_traits<TCHAR>::length(&buf[i]), i < buf.size() && buf[i];
       i += n + 1) {
...
class Results : memheap_vector<SearchResult> {
  ...
  typedef std::vector<
      std::pair<Indexes::value_type::value_type *, SearchResult::index_type>>
      IndicesInUse;
...
public:
  ...
  SearchResult::index_type
  save_index(Indexes::value_type::element_type *const index) {
    IndicesInUse::iterator j = std::lower_bound(
        this->indices_in_use.begin(), this->indices_in_use.end(),
        std::make_pair(index, IndicesInUse::value_type::second_type()));
    if (j == this->indices_in_use.end() || j->first != index) {
      this->indexes.push_back(index);
      j = this->indices_in_use.insert(
          j, IndicesInUse::value_type(
  ...
  Indexes::value_type::element_type *
  ith_index(value_type::index_type const i) const {
  ...
  void push_back(Indexes::value_type::element_type *const index,
...
long long get_time_zone_bias() {
  ...
  long long ft_local;
  if (!FileTimeToLocalFileTime(&reinterpret_cast<FILETIME &>(ft),
...
template <class Container> struct NFormatBase {
  typedef basic_iterator_ios<std::back_insert_iterator<Container>,
                             typename Container::traits_type>
      type;
};
class NFormat : public NFormatBase<std::tstring>::type,
                public NFormatBase<std::tvstring>::type {
...
public:
  ...
  template <class T> struct lazy {
    ...
    operator std::tstring() const {
      std::tstring result;
      me->NFormatBase<std::tstring>::type::put(std::back_inserter(result),
                                               *value);
      return result;
    }
    operator std::tvstring() const {
      std::tvstring result;
      me->NFormatBase<std::tvstring>::type::put(std::back_inserter(result),
                                                *value);
      return result;
    }
    template <class String>
    friend String &operator+=(String &out, lazy const &this_) {
      this_.me->NFormatBase<String>::type::put(std::back_inserter(out),
                                               *this_.value);
      return out;
    }
  };
  template <class T> lazy<T> operator()(T const &value) const {
...
unsigned short get_subsystem(IMAGE_DOS_HEADER const *const image_base) {
  return reinterpret_cast<IMAGE_NT_HEADERS const *>(
             reinterpret_cast<unsigned char const *>(image_base) +
             image_base->e_lfanew)
...
struct HookedNtUserProps {
  ...
  struct : hook_detail::thread_hook_swap<HOOK_TYPE(NtUserGetProp)> {
    HANDLE operator()(HWND hWnd, ATOM PropId) override {
      hook_type *const self =
          CONTAINING_RECORD(this, hook_type, HOOK_CONCAT(hook_, NtUserGetProp));
  ...
  struct : hook_detail::thread_hook_swap<HOOK_TYPE(NtUserSetProp)> {
    BOOL operator()(HWND hWnd, ATOM PropId, HANDLE value) override {
      hook_type *const self =
          CONTAINING_RECORD(this, hook_type, HOOK_CONCAT(hook_, NtUserSetProp));
...
class CMainDlg : public CModifiedDialogImpl<CMainDlg>,
                 public WTL::CDialogResize<CMainDlg>,
                 public CInvokeImpl<CMainDlg>,
...
#pragma warning(suppress : 4555)
  ...
  public:
    ...
    LRESULT OnKey(UINT uMsg, WPARAM wParam, LPARAM lParam) {
      if ((wParam == VK_UP || wParam == VK_DOWN) ||
        ...
        KeyNotify msg = {{*this, static_cast<unsigned int>(id),
                          static_cast<unsigned int>(
                              uMsg == WM_KEYUP ? CUN_KEYUP : CUN_KEYDOWN)},
                         wParam,
                         lParam};
        HWND hWndParent = this->GetParent();
        return hWndParent == NULL || this->SendMessage(hWndParent, WM_NOTIFY,
                                                       id, (LPARAM)&msg)
    ...
    void EnsureTrackingMouseHover() {
      if (!this->tracking) {
        TRACKMOUSEEVENT tme = {sizeof(tme), TME_HOVER | TME_LEAVE, this->m_hWnd,
                               0};
    ...
    void OnMouseHover(WPARAM /*wParam*/, WTL::CPoint /*ptPos*/) {
      ...
      WTL::CString sysdir;
      {
        ...
        unsigned int const cch =
            GetWindowsDirectory(buf, sysdir.GetAllocLength());
        sysdir.Delete(cch, sysdir.GetLength() - static_cast<int>(cch));
      }
      WTL::CString const title = this->LoadString(IDS_SEARCH_PATTERN_TITLE),
                         body = this->LoadString(IDS_SEARCH_PATTERN_BODY) +
                                _T("\r\n") + sysdir + getdirsep() + _T("**") +
                                getdirsep() + _T("*.exe") + _T("\r\n") +
                                _T("Picture*.jpg");
  ...
  struct CacheInfo {
    explicit CacheInfo(size_t const counter)
        : counter(counter), valid(false), iIconSmall(-1), iIconLarge(-1),
          iIconExtraLarge(-1) {
  ...
  enum {
    ...
    WM_READY = WM_NOTIFYICON + 1,
    WM_REFRESHITEMS = WM_READY + 1
  };

  typedef std::map<std::tvstring, CacheInfo> ShellInfoCache;
  typedef std::map<std::tvstring, std::tvstring> TypeInfoCache;

  template <class StrCmp> class NameComparator {
    StrCmp less;

  public:
    NameComparator(StrCmp const &less) : less(less) {}
    bool operator()(Results::value_type const &a,
                    Results::value_type const &b) {
    ...
    bool operator()(Results::value_type const &a,
                    Results::value_type const &b) const {
      bool less = this->less(a.file_name(), b.file_name());
      if (!less && !this->less(b.file_name(), a.file_name())) {
        less = this->less(a.stream_name(), b.stream_name());
      }
  ...
  mutable TypeInfoCache
      type_cache /* based on file type ONLY; may be accessed concurrently */;
  ...
  Results results;
  WTL::CImageList
      _small_image_list; // image list that is used as the "small" image list
  WTL::CImageList imgListSmall, imgListLarge,
      imgListExtraLarge; // lists of small images
  ...
  static DWORD WINAPI SHOpenFolderAndSelectItemsThread(IN LPVOID lpParameter) {
    std::auto_ptr<
        std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
                  std::vector<CShellItemIDList>>>
        p(static_cast<
            std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
                      std::vector<CShellItemIDList>> *>(lpParameter));
    // This is in a separate thread because of a BUG:
    // Try this with RmMetadata:
    // 1. Double-click it.
    // 2. Press OK when the error comes up.
    // 3. Now you can't access the main window, because
    // SHOpenFolderAndSelectItems() hasn't returned! So we put this in a
    // separate thread to solve that problem.
    ...
    return SHOpenFolderAndSelectItems(
        p->first.first, static_cast<UINT>(relative_item_ids.size()),
...
public:
  CMainDlg(HANDLE const hEvent, bool const rtl)
      : CModifiedDialogImpl<CMainDlg>(true, rtl),
        ...
        nformat_ui(std::locale("")), nformat_io(std::locale()),
        lcid(GetThreadLocale()), hEvent(hEvent), deletedColor(RGB(0xFF, 0, 0)),
        encryptedColor(RGB(0, 0xFF, 0)), compressedColor(RGB(0, 0, 0xFF)) {
    this->time_zone_bias = get_time_zone_bias();
  }

  void SystemTimeToString(long long system_time /* UTC */,
                          std::tvstring &buffer, bool const sortable,
                          bool const include_time = true) const {
    return SystemTimeToStringImpl(system_time, buffer, sortable, include_time,
                                  this->time_zone_bias, this->lcid);
  }

  static void SystemTimeToStringImpl(long long system_time /* UTC */,
                                     std::tvstring &buffer, bool const sortable,
                                     bool const include_time,
                                     long long const time_zone_bias,
                                     LCID const lcid) {
    long long local_time = system_time + time_zone_bias;
    winnt::TIME_FIELDS tf;
    winnt::RtlTimeToTimeFields(&reinterpret_cast<LARGE_INTEGER &>(local_time),
                               &tf);
    if (sortable) {
      TCHAR buf[64], *p = buf;
      size_t cch_zero;
      ...
      SYSTEMTIME sysTime = {
          static_cast<WORD>(tf.Year),    static_cast<WORD>(tf.Month),
          static_cast<WORD>(tf.Weekday), static_cast<WORD>(tf.Day),
          static_cast<WORD>(tf.Hour),    static_cast<WORD>(tf.Minute),
          static_cast<WORD>(tf.Second),  static_cast<WORD>(tf.Milliseconds)};
      TCHAR buf[64];
      size_t const buffer_size = sizeof(buf) / sizeof(*buf);
      size_t cch = 0;
      size_t const cchDate = static_cast<size_t>(GetDateFormat(
          lcid, 0, &sysTime, NULL, &buf[0], static_cast<int>(buffer_size)));
      cch += cchDate - !!cchDate /* null terminator */;
      if (cchDate > 0 && include_time) {
        buf[cch++] = _T(' ');
        size_t const cchTime = static_cast<size_t>(
            GetTimeFormat(lcid, 0, &sysTime, NULL, &buf[cchDate],
                          static_cast<int>(buffer_size - cchDate)));
        cch += cchTime - !!cchTime;
      }
  ...
  struct IconLoaderCallback {
      ...
      bool operator()() {
        ...
        if (cached != this_->cache.end()) {
          ...
          if (cached->second.iIconSmall < 0) {
            ...
            this_->imgListSmall.ReplaceIcon(cached->second.iIconSmall,
                                            iconSmall);
          }
          ...
          if (cached->second.iIconLarge < 0) {
            ...
            this_->imgListLarge.ReplaceIcon(cached->second.iIconLarge,
                                            iconLarge);
          }
    ...
    BOOL operator()() {
      ...
      this_->lvFiles.GetItemRect(
          iItem, &rcItem, LVIR_BOUNDS); // Blocks, but I'm hoping it's fast...
      bool const still_visible =
          !!IntersectRect(&intersection, &rcFiles, &rcItem);
      {
...
#endif
        Handle
            fileTemp; // To prevent icon retrieval from changing the file time
        SetLastError(0);
        WTL::CIcon iconSmall, iconLarge;
        ...
        BOOL success = FALSE;
        {
          for (int pass = 0; attempt && pass < 2; ++pass) {
            ...
            ULONG const flags =
                ...
                SHGFI_ADDOVERLAYS | // SHGFI_TYPENAME | SHGFI_SYSICONINDEX |
                (pass ? SHGFI_LARGEICON : SHGFI_SMALLICON);
            // CoInit com;  // MANDATORY!  Some files, like '.sln' files, won't
            // work without it!
            Wow64Disable const wow64disable(true);
            success = SHGetFileInfo(normalizedPath.c_str(), fileAttributes,
                                    &shfi, sizeof(shfi), flags) != 0;
          ...
          MainThreadCallback callback = {this_,
                                         description,
                                         path_copy,
                                         iconSmall.Detach(),
                                         iconLarge.Detach(),
  ...
  int CacheIcon(std::tvstring path, int const iItem, ULONG fileAttributes,
    ...
    if (!already_in_cache) {
      ...
      size_t max_possible_icons =
          MulDiv(rcClient.Width(), rcClient.Height(),
      ...
      if (current_cache_size >= 2 * max_cache_size) {
        for (ShellInfoCache::iterator i = this->cache.begin();
             i != this->cache.end();) {
      ...
      IconLoaderCallback callback = {
          this, path, iconSmallSize, iconSmallLarge, fileAttributes, iItem};
  ...
  WTL::CImageList small_image_list() const { return this->_small_image_list; }
  ...
  BOOL OnInitDialog(CWindow /*wndFocus*/, LPARAM /*lInitParam*/) {
    ...
    this->txtPattern.SetCueBannerText(
        this->LoadString(IDS_SEARCH_PATTERN_BANNER), true);
    WTL::CHeaderCtrl hdr = this->lvFiles.GetHeader();
    {
      int const icol = COLUMN_INDEX_NAME;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 192,
                         this->LoadString(IDS_COLUMN_NAME_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_PATH;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 265,
                         this->LoadString(IDS_COLUMN_PATH_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_TYPE;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 80,
                         this->LoadString(IDS_COLUMN_TYPE_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_SIZE;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_RIGHT, 115,
                         this->LoadString(IDS_COLUMN_SIZE_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_SIZE_ON_DISK;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_RIGHT, 115,
                         this->LoadString(IDS_COLUMN_SIZE_ON_DISK_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_CREATION_TIME;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 86,
                         this->LoadString(IDS_COLUMN_CREATION_TIME_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_MODIFICATION_TIME;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 86,
                         this->LoadString(IDS_COLUMN_WRITE_TIME_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_ACCESS_TIME;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_LEFT, 86,
                         this->LoadString(IDS_COLUMN_ACCESS_TIME_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }
    {
      int const icol = COLUMN_INDEX_DESCENDENTS;
      LVCOLUMN column = {LVCF_FMT | LVCF_WIDTH | LVCF_TEXT, LVCFMT_RIGHT, 74,
                         this->LoadString(IDS_COLUMN_DESCENDENTS_HEADER)};
      this->lvFiles.InsertColumn(icol, &column);
    }

    this->cmbDrive.SetCueBannerText(this->LoadString(IDS_SEARCH_VOLUME_BANNER));
    HINSTANCE hInstance = GetModuleHandle(NULL);
    this->SetIcon((HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1),
...

    {
      ...
      this->imgListSmall.Create(GetSystemMetrics(SM_CXSMICON),
                                GetSystemMetrics(SM_CXSMICON), ILC_COLOR32, 0,
                                IMAGE_LIST_INCREMENT);
      this->imgListLarge.Create(GetSystemMetrics(SM_CXICON),
                                GetSystemMetrics(SM_CXICON), ILC_COLOR32, 0,
                                IMAGE_LIST_INCREMENT);
      this->imgListExtraLarge.Create(48, 48, ILC_COLOR32, 0,
                                     IMAGE_LIST_INCREMENT);
    }
    ...
    if (false) {
      ...
      if (font.GetLogFont(logFont)) {
        ...
        this->txtPattern.SetFont(
            WTL::CFontHandle().CreateFontIndirect(&logFont));
      }
    }
    this->trie_filtering = this->GetMenu().GetMenuState(
                               ID_OPTIONS_FAST_PATH_SEARCH, MF_BYCOMMAND) &
                           MFS_CHECKED;
    this->lvFiles.SetExtendedListViewStyle(
        LVS_EX_DOUBLEBUFFER | LVS_EX_FULLROWSELECT | LVS_EX_LABELTIP |
        (this->GetMenu().GetMenuState(ID_VIEW_GRIDLINES, MF_BYCOMMAND)
             ? LVS_EX_GRIDLINES
             : 0) |
        LVS_EX_HEADERDRAGDROP | 0x80000000 /*LVS_EX_COLUMNOVERFLOW*/);
    {
      this->small_image_list(
          (this->GetMenu().GetMenuState(ID_VIEW_LARGEICONS, MF_BYCOMMAND) &
           MFS_CHECKED)
              ? this->imgListLarge
              : this->imgListSmall);
    ...
    this->statusbar = CreateStatusWindow(WS_CHILD | SBT_TOOLTIPS | WS_VISIBLE,
                                         NULL, *this, IDC_STATUS_BAR);
    int const rcStatusPaneWidths[] = {360, -1};
    this->statusbar.SetParts(sizeof(rcStatusPaneWidths) /
                                 sizeof(*rcStatusPaneWidths),
    ...
    // this->statusbarProgress.Create(this->statusbar, rcStatusPane1, NULL,
    // WS_CHILD | WS_VISIBLE | PBS_SMOOTH, 0);
    ...
    if (this->lvFiles.GetClientRect(&clientRect)) {
      ...
      this->lvFiles.ResizeClient(clientRect.Width(), clientRect.Height(),
                                 FALSE);
    }

    WTL::CRect my_client_rect;
    bool const unresize = this->GetClientRect(&my_client_rect) &&
                          this->template_size.cx > 0 &&
                          this->template_size.cy > 0 &&
                          my_client_rect.Size() != this->template_size &&
                          this->ResizeClient(this->template_size.cx,
                                             this->template_size.cy, FALSE);
    this->DlgResize_Init(false, false);
    if (unresize) {
      this->ResizeClient(my_client_rect.Width(), my_client_rect.Height(),
                         FALSE);
    }
    ...
    this->cmbDrive.SetCurSel(
        this->cmbDrive.AddString(this->LoadString(IDS_SEARCH_VOLUME_ALL)));
  ...
  DWORD GetDlgTemplate(LPDLGTEMPLATE buffer, DWORD cbLen) {
    DWORD cb =
        this->CModifiedDialogImpl<CMainDlg>::GetDlgTemplate(buffer, cbLen);
    if (cb <= cbLen) {
...
#pragma pack(push, 1)
    struct result_type {
      ...
      result_type(first_type const &first, second_type const &second)
...
#pragma pack(pop)
    ...
    void check_cancelled(SearchResult const *const a = NULL,
                         SearchResult const *const b = NULL) {
      ++numerator;
      if (dlg) {
        ...
        if (dlg->ShouldUpdate(tnow)) {
          ...
          if (a) {
            Index const *const p =
                this_->results.ith_index(a->index())->unvolatile();
            buffer += p->root_path();
          ...
          if (b) {
            Index const *const p =
                this_->results.ith_index(b->index())->unvolatile();
            buffer += p->root_path();
            p->get_path(b->key(), buffer, false);
          }
          dlg->SetProgressText(buffer);
          dlg->SetProgress(numerator,
  ...
  template <int SubItem> struct ResultCompare {
    ...
    result_type operator()(Results::value_type const &v) const {
      ...
      NtfsIndex const *const p =
          base->this_->results.ith_index(v.index())->unvolatile();
      ...
      case COLUMN_INDEX_DESCENDENTS: {
        ...
        case COLUMN_INDEX_SIZE:
          result.second =
              (base->variation & 0x1)
                  ? /* sort by space saved */ static_cast<unsigned long long>(
                        info.length - info.allocated) -
                        (1ULL << ((sizeof(unsigned long long) * CHAR_BIT) - 1))
                  : static_cast<unsigned long long>(info.length);
          break;
        case COLUMN_INDEX_SIZE_ON_DISK:
          result.second =
              (base->variation & 0x1) ? info.bulkiness : info.allocated;
          break;
        case COLUMN_INDEX_DESCENDENTS:
    ...
    int precompare(Results::value_type const &a,
                   Results::value_type const &b) const {
      int r = 0;
      if (base->variation & 0x2) {
        size_t const a_depth = a.depth(), b_depth = b.depth();
        if (a_depth < b_depth) {
          r = -1;
    ...
    bool operator()(first_argument_type const &a,
    ...
    bool operator()(Results::value_type const &a,
      ...
      if (precomp < 0) {
        ...
        case COLUMN_INDEX_TYPE: {
          bool const name_only = SubItem != COLUMN_INDEX_PATH,
                     is_type = SubItem == COLUMN_INDEX_TYPE;
          int c;
          c = 0;
          if (!name_only) {
            Index::ParentIterator::value_type_compare comp;
            std::tvstring const &r1 = index1->root_path(),
                                &r2 = index2->root_path();
            Index::ParentIterator::value_type v1 = {r1.data()},
                                              v2 = {r2.data()};
            v1.second = r1.size();
            v1.ascii = false;
            v2.second = r2.size();
          ...
          if (c != 0) {
            ...
            if (!name_only) {
              Index::ParentIterator j1(index1, a.key()), j2(index2, b.key());
              {
                ...
                Index::ParentIterator *const ideeper =
                    &(a_depth < b_depth ? j2 : j1);
                for (unsigned short depthdiff = a_depth < b_depth
                                                    ? b_depth - a_depth
                                                    : a_depth - b_depth;
                     depthdiff; --depthdiff) {
                  ++*ideeper;
            ...
            if (is_type) {
              ...
              i1.next();
              append_directional(base->buffer2,
                                 static_cast<TCHAR const *>(i1->first),
                                 i1->second, i1->ascii ? -1 : 0, false);
              size_t const k1 = k0 + base->this_->get_file_type_blocking(
                                         base->buffer2, k0, i1.attributes());
              base->buffer2 += index2->root_path();
              i2.next();
              append_directional(base->buffer2,
                                 static_cast<TCHAR const *>(i2->first),
                                 i2->second, i2->ascii ? -1 : 0, false);
              size_t const k2 = k1 + base->this_->get_file_type_blocking(
                                         base->buffer2, k1, i2.attributes());
              less = std::lexicographical_compare(
                  base->buffer2.begin() + static_cast<ptrdiff_t>(k0),
                  base->buffer2.begin() + static_cast<ptrdiff_t>(k1),
                  base->buffer2.begin() + static_cast<ptrdiff_t>(k1),
                  base->buffer2.begin() + static_cast<ptrdiff_t>(k2));
            } else {
              size_t itemp = 0;
              base->temp->resize((1 + USHRT_MAX) * 2);
              while (i1.icomponent() != n1 && i1.next() &&
                     !(name_only && i1.icomponent())) {
                (*base->temp)[itemp++] = *i1;
              }
              size_t const len1 = itemp;
              while (i2.icomponent() != n2 && i2.next() &&
                     !(name_only && i2.icomponent())) {
                (*base->temp)[itemp++] = *i2;
              }
              // Here we rely on the path structure, assuming that components
              // are broken down on well-defined boundaries. This would NOT work
              // with arbitrary substrings.
              less = std::lexicographical_compare(
                  base->temp->rend() - static_cast<ptrdiff_t>(len1),
                  base->temp->rend(),
                  base->temp->rend() - static_cast<ptrdiff_t>(itemp),
                  base->temp->rend() - static_cast<ptrdiff_t>(len1),
  ...
  LRESULT OnFilesListColumnClick(LPNMHDR pnmh) {
    ...
    unsigned char const variation =
        (alt_pressed ? 1U : 0U) | ((shift_pressed ? 1U : 0U) << 1);
    bool cancelled = false;
    this->lvFiles.SetItemState(-1, 0, LVNI_SELECTED);
    int const subitem = pLV->iSubItem;
    bool const same_key_as_last = this->last_sort.column == subitem + 1 &&
    ...
    if ((this->lvFiles.GetStyle() & LVS_OWNERDATA) != 0) {
      try {
        ...
        TCHAR buf[0x100];
        safe_stprintf(
            buf, this->LoadString(IDS_STATUS_SORTING_RESULTS),
            static_cast<std::tstring>(nformat_ui(this->results.size()))
                .c_str());
        ...
        if (!dlg.HasUserCancelled()) {
          ...
          std::vector<unsigned long long> temp_keys;
          Results::iterator const begin = this->results.begin(),
                                  end = this->results.end();
          ptrdiff_t const item_count = end - begin;
          using std::ceil;
          ResultCompareBase compare = {
              this,
              &temp,
              &temp_keys,
              variation,
              &dlg,
              static_cast<unsigned long long>(
                  (ceil(log(static_cast<double>(item_count)) + 1) * item_count *
                   1.5))};
...
#define X(Column)                                                              \
  case Column: {                                                               \
    pretend_reversed = !reversed;                                              \
    proposed_sort = pretend_reversed ? +1 : -1;                                \
    ResultCompare<Column> comp(&compare);                                      \
    if (is_sorted_ex(begin, end, comp, pretend_reversed)) {                    \
...
#define X(Column)                                                              \
  case Column: {                                                               \
    ...
    if (is_sorted_ex(begin, end, comp, pretend_reversed)) {                    \
      ...
      stable_sort_by_key(begin, end, comp, compare.swapper());                 \
      if (!pretend_reversed) {                                                 \
        std::reverse(begin, end);                                              \
      }                                                                        \
    }                                                                          \
...
#undef X
          ...
          safe_stprintf(
              buf, this->LoadString(IDS_STATUS_SORTED_RESULTS),
              static_cast<std::tstring>(nformat_ui(this->results.size()))
                  .c_str(),
              (tend - tstart) * 1.0 / CLOCKS_PER_SEC);
          this->statusbar.SetText(0, buf);
        }
      } catch (CStructured_Exception &ex) {
        cancelled = true;
    ...
    if (!cancelled) {
      this->last_sort.counter =
          same_key_as_last ? this->last_sort.counter + 1 : 1;
    ...
    if (header.GetItem(subitem, &hditem)) {
      hditem.fmt = (hditem.fmt & ~(HDF_SORTUP | HDF_SORTDOWN)) |
                   (resulting_sort < 0   ? HDF_SORTDOWN
                    : resulting_sort > 0 ? HDF_SORTUP
                                         : 0);
  ...
  BOOL SetItemCount(int n, bool const assume_existing_are_valid = false) {
    ...
    if (this->lvFiles.GetStyle() & LVS_OWNERDATA) {
      ...
      if (n == 0) {
        ...
        result =
            assume_existing_are_valid ? TRUE : this->lvFiles.DeleteAllItems();
        ...
        for (; result && i < n; ++i) {
          LVITEM item;
          item.iItem = i;
          for (item.iSubItem = 0; result && item.iSubItem < nsubitems;
               ++item.iSubItem) {
            if (item.iSubItem != COLUMN_INDEX_TYPE) {
              item.mask = {LVIF_TEXT};
              this->get_subitem_text(static_cast<size_t>(item.iItem),
                                     item.iSubItem, text);
              item.pszText = (text.c_str() /* ensure null-terminated */,
                              text.empty() ? NULL : &*text.begin());
  ...
  void Search() {
    ...
    std::vector<index_pointer> nonwait_indices, wait_indices,
        initial_wait_indices;
    ...
    if (preallocate) {
      try {
        this->results.reserve(this->results.size() + expected_results +
                              expected_results / 8);
      } catch (std::bad_alloc &) {
      }
    }
    std::vector<IoPriority> set_priorities(nonwait_indices.size() +
                                           wait_indices.size());
    for (size_t i = 0; i != nonwait_indices.size(); ++i) {
      IoPriority(reinterpret_cast<uintptr_t>(nonwait_indices[i]->volume()),
                 winnt::IoPriorityLow)
          .swap(set_priorities.at(i));
    }
    for (size_t i = 0; i != wait_indices.size(); ++i) {
      IoPriority(reinterpret_cast<uintptr_t>(wait_indices[i]->volume()),
                 winnt::IoPriorityLow)
          .swap(set_priorities.at(nonwait_indices.size() + i));
    }
    ...
    while (!wait_handles.empty()) {
      if (uintptr_t const volume =
              reinterpret_cast<uintptr_t>(wait_indices.at(0)->volume())) {
        if (set_priority.volume() != volume) {
          IoPriority(volume, winnt::IoPriorityNormal).swap(set_priority);
        }
      }
      unsigned long const wait_result = dlg.WaitMessageLoop(
          wait_handles.empty() ? NULL : &*wait_handles.begin(),
          wait_handles.size());
      if (wait_result == WAIT_TIMEOUT) {
        ...
        if (dlg.ShouldUpdate()) {
          ...
          ss << this->LoadString(IDS_TEXT_READING_FILE_TABLES)
             << this->LoadString(IDS_TEXT_SPACE);
          bool any = false;
          unsigned long long temp_overall_progress_numerator =
              overall_progress_numerator;
          for (size_t i = 0; i != wait_indices.size(); ++i) {
            ...
            if (records_so_far != mft_capacity) {
              if (any) {
                ss << this->LoadString(IDS_TEXT_COMMA)
                   << this->LoadString(IDS_TEXT_SPACE);
              } else {
                ss << this->LoadString(IDS_TEXT_SPACE);
              }
              ss << j->root_path() << this->LoadString(IDS_TEXT_SPACE)
                 << this->LoadString(IDS_TEXT_PAREN_OPEN)
                 << nformat_ui(j->records_so_far());
              // TODO: 'of' _really_ isn't a good thing to localize in
              // isolation..
              ss << this->LoadString(IDS_TEXT_SPACE)
                 << this->LoadString(IDS_TEXT_OF)
                 << this->LoadString(IDS_TEXT_SPACE);
              // These MUST be separate statements since nformat_ui is used
              // twice
              ss << nformat_ui(mft_capacity)
                 << this->LoadString(IDS_TEXT_PAREN_CLOSE);
              any = true;
            }
          }
          ...
          if (average_speed.first > initial_average_amount) {
            ss << _T('\n');
            ss << this->LoadString(IDS_TEXT_AVERAGE_SPEED)
               << this->LoadString(IDS_TEXT_COLON)
               << this->LoadString(IDS_TEXT_SPACE)
               << nformat_ui(static_cast<size_t>(
                      (average_speed.first - initial_average_amount) *
                      static_cast<double>(CLOCKS_PER_SEC) /
                      ((tnow != initial_time ? tnow - initial_time : 1) *
                       (1ULL << 20))))
               << this->LoadString(IDS_TEXT_SPACE)
               << this->LoadString(IDS_TEXT_MIB_S);
            ss << this->LoadString(IDS_TEXT_SPACE);
            // These MUST be separate statements since nformat_ui is used twice
            ss << this->LoadString(IDS_TEXT_PAREN_OPEN)
               << nformat_ui(average_speed.first / (1 << 20))
               << this->LoadString(IDS_TEXT_SPACE)
               << this->LoadString(IDS_TEXT_MIB_READ)
               << this->LoadString(IDS_TEXT_PAREN_CLOSE);
          }
          std::tstring const text = ss.str();
          dlg.SetProgressText(text);
          dlg.SetProgress(
              static_cast<long long>(temp_overall_progress_numerator),
        ...
        if (wait_result < wait_handles.size()) {
          ...
          size_t const current_progress_denominator =
              i->total_names_and_streams();
          ...
          if (task_result != 0) {
            if (selected != 0) {
              (dlg.IsWindow() ? ATL::CWindow(dlg.GetHWND()) : *this)
                  .MessageBox(GetAnyErrorText(task_result),
                              this->LoadString(IDS_ERROR_TITLE),
                              MB_OK | MB_ICONERROR);
            }
          }
          try {
            lock(i)->matches(
                [&dlg, &results_at_depths, &root_path, shift_pressed, this, i,
                 &wait_indices, any_io_pending, &current_progress_numerator,
                 current_progress_denominator, overall_progress_numerator,
                 overall_progress_denominator,
                 &matchop](TCHAR const *const name, size_t const name_length,
                           bool const ascii, NtfsIndex::key_type const &key,
                           size_t const depth) {
                  unsigned long const now = GetTickCount();
                  if (dlg.ShouldUpdate(now) ||
                      current_progress_denominator -
                              current_progress_numerator <=
                          1) {
                    if (dlg.HasUserCancelled(now)) {
                      throw CStructured_Exception(ERROR_CANCELLED, NULL);
                    }
                    // this->lvFiles.SetItemCountEx(static_cast<int>(this->results.size()),
                    // 0), this->lvFiles.UpdateWindow();
                    size_t temp_overall_progress_numerator =
                        overall_progress_numerator;
                    if (any_io_pending) {
                    ...
                    text.resize(static_cast<size_t>(_sntprintf(
                        ...
                        static_cast<std::tstring>(
                            nformat_ui(current_progress_numerator))
                            .c_str(),
                        this->LoadString(IDS_TEXT_SPACE).c_str(),
                        this->LoadString(IDS_TEXT_OF).c_str(),
                        this->LoadString(IDS_TEXT_SPACE).c_str(),
                        static_cast<std::tstring>(
                            nformat_ui(current_progress_denominator))
                            .c_str(),
                        this->LoadString(IDS_TEXT_PAREN_CLOSE).c_str(),
                        this->LoadString(IDS_TEXT_ELLIPSIS).c_str())));
                    if (name_length) {
                      append_directional(text, name, name_length,
                                         ascii ? -1 : 0);
                    }
                    dlg.SetProgressText(text);
                    dlg.SetProgress(
                        temp_overall_progress_numerator +
                            static_cast<unsigned long long>(i->mft_capacity) *
                                static_cast<unsigned long long>(
                                    current_progress_numerator) /
                                static_cast<unsigned long long>(
                                    current_progress_denominator),
                  ...
                  ++current_progress_numerator;
                  if (current_progress_numerator >
                      current_progress_denominator) {
                    throw std::logic_error("current_progress_numerator > "
                                           "current_progress_denominator");
                  }
                  TCHAR const *const path_begin = name;
                  size_t high_water_mark = 0,
                         *phigh_water_mark =
                             matchop.is_path_pattern && this->trie_filtering
                                 ? &high_water_mark
                                 : NULL;
                  bool match =
                      ascii ? matchop.matcher.is_match(
                                  static_cast<char const *>(
                                      static_cast<void const *>(path_begin)),
                                  name_length, phigh_water_mark)
                            : matchop.matcher.is_match(path_begin, name_length,
                                                       phigh_water_mark);
                  if (match && (matchop.size_min > 0 ||
                                matchop.size_max <
                                    static_cast<unsigned long long>(-1LL))) {
                    unsigned long long const file_size =
                        static_cast<unsigned long long>(
                            i->get_sizes(key).length);
                    if (file_size < matchop.size_min ||
                        file_size > matchop.size_max) {
                      match = false;
                    }
                  }
                  if (match && !matchop.content_query_ascii.empty()) {
                    if ((lock(i)->get_stdinfo(key.frs()).attributes() &
                         FILE_ATTRIBUTE_DIRECTORY) == 0) {
                      std::tvstring full_path = root_path;
                      lock(i)->get_path(key, full_path, false);
                      FILE *f = _tfopen(full_path.c_str(), _T("rb"));
                      if (f) {
                        fseek(f, 0, SEEK_END);
                        long fsize = ftell(f);
                        fseek(f, 0, SEEK_SET);
                        if (fsize > 0 && fsize < 50 * 1024 * 1024) {
                          ...
                          if (std::search(buf.begin(), buf.end(),
                                          matchop.content_query_ascii.begin(),
                                          matchop.content_query_ascii.end()) ==
                              buf.end()) {
                            match = false;
                          }
                        }
                  ...
                  if (match) {
                    unsigned short const depth2 = static_cast<unsigned short>(
                        depth * 2U) /* dividing by 2 later should not mess up
                                       the actual depths; it should only affect
                                       files vs. directory sub-depths */
                        ;
                    if (shift_pressed) {
                      if (depth2 >= results_at_depths.size()) {
                        results_at_depths.resize(depth2 + 1);
                      }
                      results_at_depths[depth2].push_back(
                          i, Results::value_type(key, depth2));
                    } else {
                      this->results.push_back(i,
                                              Results::value_type(key, depth2));
                    }
                  }
                  return match ||
                         !(matchop.is_path_pattern && phigh_water_mark &&
                           *phigh_water_mark < name_length);
                },
                current_path, matchop.is_path_pattern,
                matchop.is_stream_pattern, control_pressed);
          } catch (CStructured_Exception &ex) {
            if (ex.GetSENumber() != ERROR_CANCELLED) {
              throw;
            }
          ...
          if (current_progress_denominator) {
            overall_progress_numerator += static_cast<size_t>(
                static_cast<unsigned long long>(i->mft_capacity) *
                static_cast<unsigned long long>(current_progress_numerator) /
                static_cast<unsigned long long>(current_progress_denominator));
          }
        }
        wait_indices.erase(wait_indices.begin() +
                           static_cast<ptrdiff_t>(wait_result));
        wait_handles.erase(wait_handles.begin() +
                           static_cast<ptrdiff_t>(wait_result));
      }
    }
    {
      ...
      for (size_t j = results_at_depths.size(); j != 0 && ((void)--j, true);) {
        ...
        for (size_t i = results_at_depth.size(); i != 0 && ((void)--i, true);) {
          this->results.push_back(results_at_depth.item_index(i),
                                  results_at_depth[i]);
        }
      }
    ...
    TCHAR buf[0x100];
    safe_stprintf(
        buf, this->LoadString(IDS_STATUS_FOUND_RESULTS),
        static_cast<std::tstring>(nformat_ui(this->results.size())).c_str(),
        (tend - tstart) * 1.0 / CLOCKS_PER_SEC);
    this->statusbar.SetText(0, buf);
  }
  ...
  void OnBrowse(UINT /*uNotifyCode*/, int /*nID*/, HWND /*hWnd*/) {
    struct Callback {
      ...
      int operator()(ATL::CWindow window, UINT uMsg, LPARAM lParam) {
        ...
        case BFFM_INITIALIZED:
          window.SendMessage(
              BFFM_SETSELECTION, TRUE,
              reinterpret_cast<LPARAM>(this->initial_folder.c_str()));
          break;
    ...
    if (this->txtPattern.GetWindowText(bstr.m_str)) {
      ...
      while (callback.initial_folder.size() > 0 &&
             *(callback.initial_folder.end() - 1) == _T('*')) {
        callback.initial_folder.erase(callback.initial_folder.end() - 1,
                                      callback.initial_folder.end());
      }
    }
    std::tstring path(MAX_PATH, _T('\0'));
    CoInit const coinit;
    BROWSEINFO info = {this->m_hWnd,
                       NULL,
                       &*path.begin(),
                       this->LoadString(IDS_BROWSE_BODY),
  ...
  void OnOK(UINT /*uNotifyCode*/, int /*nID*/, HWND /*hWnd*/) {
    if (GetFocus() == this->lvFiles) {
      ...
      if (index >= 0 &&
          (this->lvFiles.GetItemState(index, LVNI_SELECTED) & LVNI_SELECTED)) {
  ...
  void append_selected_indices(std::vector<size_t> &result) const {
    ...
    result.reserve(result.size() +
                   static_cast<size_t>(this->lvFiles.GetSelectedCount()));
    WNDPROC const lvFiles_wndproc = reinterpret_cast<WNDPROC>(
        ::GetWindowLongPtr(this->lvFiles.m_hWnd, GWLP_WNDPROC));
    for (int i = -1;;) {
      i = static_cast<int>(lvFiles_wndproc(this->lvFiles.m_hWnd,
                                           LVM_GETNEXTITEM, i,
  ...
  class LockedResults {
    ...
    CMainDlg *me;
    IndicesLocks
        indices_locks; // this is to permit us to call unvolatile() below
  public:
    explicit LockedResults(CMainDlg &me) : me(&me) {}
    void operator()(size_t const index) {
      atomic_namespace::recursive_mutex *const m =
          &me->results.item_index(index)->get_mutex();
  ...
  void RightClick(std::vector<size_t> const &indices, POINT const &point,
                  int const focused) {
    Wow64Disable const wow64disable(true);
    LockedResults locked_results(*this);
    typedef std::vector<size_t> Indices;
    std::for_each<Indices::const_iterator, LockedResults &>(
        indices.begin(), indices.end(), locked_results);
    ATL::CComPtr<IShellFolder> desktop;
    ...
    std::auto_ptr<
        std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
                  std::vector<CShellItemIDList>>>
        p(new std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
                        std::vector<CShellItemIDList>>());
    p->second.reserve(
        indices.size()); // REQUIRED, to avoid copying CShellItemIDList objects
                         // (they're not copyable!)
    if (indices.size() <=
        (1 << 10)) // if not too many files... otherwise the shell context menu
                   // this will take a long time
    {
      ...
      for (size_t i = 0; i < indices.size(); ++i) {
        ...
        NtfsIndex const *const index =
            this->results.item_index(iresult)
                ->unvolatile() /* we are allowed to do this because the indices
                                  are locked */
            ;
        ...
        if (hr == S_OK) {
          ...
          while (!ILIsEmpty(static_cast<LPCITEMIDLIST>(p->first.first)) &&
                 !ILIsParent(p->first.first, itemIdList, FALSE)) {
            ILRemoveLastID(p->first.first);
          }
      ...
      if (hr == S_OK) {
        ...
        if (hr == S_OK) {
          if (!p->first.first.IsNull() &&
              !ILIsEmpty(static_cast<LPCITEMIDLIST>(p->first.first))) {
            hr = desktop->BindToObject(
                p->first.first, NULL, IID_IShellFolder,
      ...
      if (hr == S_OK) {
        bool const desktop_relative =
            p->first.first.IsNull() ||
            ILIsEmpty(static_cast<LPCITEMIDLIST>(p->first.first));
        std::vector<LPCITEMIDLIST> relative_item_ids(p->second.size());
        for (size_t i = 0; i < p->second.size(); ++i) {
          relative_item_ids[i] =
              desktop_relative ? static_cast<LPCITEMIDLIST>(p->second[i])
                               : ILFindChild(p->first.first, p->second[i]);
        }
        GUID const IID_IContextMenu = {
            0x000214E4L, 0, 0, {0xC0, 0, 0, 0, 0, 0, 0, 0x46}};
        hr = (desktop_relative ? desktop : p->first.second)
                 ->GetUIObjectOf(
                     *this, static_cast<UINT>(relative_item_ids.size()),
                     relative_item_ids.empty() ? NULL : &relative_item_ids[0],
                     IID_IContextMenu, NULL,
                     &reinterpret_cast<void *&>(contextMenu.p));
      }
      if (hr == S_OK) {
        hr = contextMenu->QueryContextMenu(menu, 0, minID, UINT_MAX,
                                           0x80 /*CMF_ITEMMENU*/);
      }
    }
    unsigned int ninserted = 0;
    UINT const openContainingFolderId = minID - 1, fileIdId = minID - 2,
               copyId = minID - 3, copyPathId = minID - 4,
               copyTableId = minID - 5, dumpId = minID - 6;

    if (indices.size() == 1) {
      MENUITEMINFO mii2 = {sizeof(mii2),
                           MIIM_ID | MIIM_STRING | MIIM_STATE,
                           MFT_STRING,
                           MFS_ENABLED,
                           openContainingFolderId,
                           NULL,
                           NULL,
                           NULL,
                           NULL,
                           this->LoadString(IDS_MENU_OPEN_CONTAINING_FOLDER)};
    ...
    if (0 <= focused && static_cast<size_t>(focused) < this->results.size()) {
      {
        ...
        text +=
            static_cast<std::tstring>(
                nformat_ui(
                    this->results[static_cast<size_t>(focused)].key().frs()))
                .c_str();
        MENUITEMINFO mii = {sizeof(mii), MIIM_ID | MIIM_STRING | MIIM_STATE,
                            MFT_STRING,  MFS_DISABLED,
                            fileIdId,    NULL,
                            NULL,        NULL,
                            NULL,        text};
      ...
      }
      {
        MENUITEMINFO mii = {sizeof(mii), MIIM_ID | MIIM_STRING | MIIM_STATE,
                            ...
                            copyId,      NULL,
                            NULL,        NULL,
                            NULL,        this->LoadString(IDS_MENU_COPY)};
        menu.InsertMenuItem(ninserted++, TRUE, &mii);
      }
      {
        MENUITEMINFO mii = {sizeof(mii), MIIM_ID | MIIM_STRING | MIIM_STATE,
                            ...
                            copyPathId,  NULL,
                            NULL,        NULL,
                            NULL,        this->LoadString(IDS_MENU_COPY_PATHS)};
        menu.InsertMenuItem(ninserted++, TRUE, &mii);
      }
      {
        MENUITEMINFO mii = {sizeof(mii), MIIM_ID | MIIM_STRING | MIIM_STATE,
                            ...
                            copyTableId, NULL,
                            NULL,        NULL,
                            NULL,        this->LoadString(IDS_MENU_COPY_TABLE)};
        menu.InsertMenuItem(ninserted++, TRUE, &mii);
      }
      {
        MENUITEMINFO mii = {
            sizeof(mii), MIIM_ID | MIIM_STRING | MIIM_STATE,
            MFT_STRING,  0,
            dumpId,      NULL,
            NULL,        NULL,
            NULL,        this->LoadString(IDS_MENU_DUMP_TO_TABLE)};
        menu.InsertMenuItem(ninserted++, TRUE, &mii);
      }
    }
    ...
    UINT id = menu.TrackPopupMenu(
        TPM_RETURNCMD | TPM_NONOTIFY |
            (GetKeyState(VK_SHIFT) < 0 ? CMF_EXTENDEDVERBS : 0) |
            (GetSystemMetrics(SM_MENUDROPALIGNMENT)
                 ? TPM_RIGHTALIGN | TPM_HORNEGANIMATION
                 : TPM_LEFTALIGN | TPM_HORPOSANIMATION),
        point.x, point.y, *this);
    if (!id) {
      // User cancelled
    } else if (id == openContainingFolderId) {
      if (QueueUserWorkItem(&SHOpenFolderAndSelectItemsThread, p.get(),
                            WT_EXECUTEINUITHREAD)) {
        p.release();
      }
    } else if (id == dumpId || id == copyId || id == copyPathId ||
               id == copyTableId) {
      this->dump_or_save(indices,
                         id == dumpId   ? 0
                         : id == copyId ? 2
                                        : 1,
                         id == dumpId || id == copyTableId ? -1
                                                           : COLUMN_INDEX_PATH);
    } else if (id >= minID) {
      CMINVOKECOMMANDINFO cmd = {
          ...
          CMIC_MASK_ASYNCOK,
          *this,
          reinterpret_cast<LPCSTR>(static_cast<uintptr_t>(id - minID)),
          NULL,
          NULL,
          SW_SHOW};
      ...
      if (hr == S_OK) {
        ...
        this->MessageBox(GetAnyErrorText(hr), this->LoadString(IDS_ERROR_TITLE),
                         MB_OK | MB_ICONERROR);
      }
    }
  }
  ...
  void
  dump_or_save(std::vector<size_t> const &locked_indices,
               int const mode /* 0 if dump to file, 1 if copy to clipboard, 2 if
                                 copy to clipboard as shell file list */
               ,
               int const single_column) {
    std::tstring file_dialog_save_options;
    if (mode <= 0) {
      std::tstring const null_char(
          1, _T('\0')); // Do NOT convert this to TCHAR because 'wchar_t' might
                        // get interpreted as 'unsigned short' depending on
                        // compiler flags
    ...
    WTL::CFileDialog fdlg(FALSE, _T("csv"), NULL,
                          OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                          file_dialog_save_options.c_str(), *this);
    fdlg.m_ofn.lpfnHook = NULL;
    fdlg.m_ofn.Flags &= ~OFN_ENABLEHOOK;
    fdlg.m_ofn.lpstrTitle = this->LoadString(IDS_SAVE_TABLE_TITLE);
    if (mode <= 0
            ? GetSaveFileName(&fdlg.m_ofn)
            : (locked_indices.size() <= 1 << 16 ||
               this->MessageBox(
                   this->LoadString(IDS_COPY_MAY_USE_TOO_MUCH_MEMORY_BODY),
                   this->LoadString(IDS_WARNING_TITLE),
                   MB_OKCANCEL | MB_ICONWARNING) == IDOK)) {
      bool const tabsep = mode > 0 || fdlg.m_ofn.nFilterIndex > 1;
      WTL::CWaitCursor wait;
      int const ncolumns = this->lvFiles.GetHeader().GetItemCount();
      File const output = {mode <= 0 ? _topen(fdlg.m_ofn.lpstrFile,
                                              _O_BINARY | _O_TRUNC | _O_CREAT |
                                                  _O_RDWR | _O_SEQUENTIAL,
                                              _S_IREAD | _S_IWRITE)
                                     : NULL};
      if (mode > 0 || output != NULL) {
        bool const shell_file_list = mode == 2;
        if (shell_file_list) {
          assert(single_column == COLUMN_INDEX_PATH &&
                 "code below assumes path column is the only one specified");
        }
        struct Clipboard {
          bool success;
          ~Clipboard() {
          ...
          explicit Clipboard(HWND owner)
              : success(owner ? !!::OpenClipboard(owner) : false) {}
        ...
        CProgressDialog dlg(*this);
        dlg.SetProgressTitle(
            this->LoadString(output ? IDS_DUMPING_TITLE : IDS_COPYING_TITLE));
        if (locked_indices.size() > 1 && dlg.HasUserCancelled()) {
          return;
        }
        std::string line_buffer_utf8;
        SingleMovableGlobalAllocator global_alloc;
        std::tvstring line_buffer(static_cast<std::tvstring::allocator_type>(
            output ? NULL : &global_alloc));
        size_t const buffer_size = 1 << 22;
        line_buffer.reserve(
            buffer_size); // this is necessary since MSVC STL reallocates
                          // poorly, degenerating into O(n^2)
        if (shell_file_list) {
          WTL::CPoint pt;
          GetCursorPos(&pt);
          DROPFILES df = {sizeof(df), pt, FALSE,
                          sizeof(*line_buffer.data()) > sizeof(char)};
          line_buffer.append(reinterpret_cast<TCHAR const *>(&df),
                             sizeof(df) / sizeof(TCHAR));
        }
        if (!output &&
            (single_column == COLUMN_INDEX_PATH || single_column < 0)) {
          line_buffer.reserve(locked_indices.size() * MAX_PATH *
                              (single_column >= 0 ? 2 : 3) / 4);
        }
        std::vector<int> displayed_columns(ncolumns, -1);
        this->lvFiles.GetColumnOrderArray(
            static_cast<int>(displayed_columns.size()),
            &*displayed_columns.begin());
        ...
        try {
          ...
          for (size_t i = 0; i < locked_indices.size(); ++i) {
            ...
            for (int c = 0; c < (single_column >= 0 ? 1 : ncolumns); ++c) {
              int const j =
                  single_column >= 0 ? single_column : displayed_columns[c];
              ...
              this->GetSubItemText(locked_indices[i], j, false, line_buffer,
                                   false);
              if (j == COLUMN_INDEX_PATH) {
                ...
                TCHAR const first_char = begin_offset < line_buffer.size()
                                             ? line_buffer[begin_offset]
                                             : _T('\0');
                // TODO: This is broken currently
                is_ads =
                    is_ads ||
                    (begin_offset <= last_colon_index &&
                     last_colon_index < line_buffer.size() &&
                     !(last_colon_index == begin_offset + 1 &&
                       ((_T('a') <= first_char && first_char <= _T('z')) ||
                        (_T('A') <= first_char && first_char <= _T('Z'))) &&
                       line_buffer.size() > last_colon_index + 1 &&
                       line_buffer.at(last_colon_index + 1) ==
                           _T('\\'))) /* TODO: This will fail if we later
                                         support paths like \\.\C: */
                    ;
                unsigned long const update_time = GetTickCount();
                if (dlg.ShouldUpdate(update_time) ||
                    i + 1 == locked_indices.size()) {
                  if (dlg.HasUserCancelled(update_time)) {
                    throw CStructured_Exception(ERROR_CANCELLED, NULL);
                  }
                  should_flush = true;
                  basic_fast_ostringstream<TCHAR> ss;
                  ss << this->LoadString(output ? IDS_TEXT_DUMPING_SELECTION
                                                : IDS_TEXT_COPYING_SELECTION)
                     << this->LoadString(IDS_TEXT_SPACE);
                  ss << nformat_ui(i + 1);
                  ss << this->LoadString(IDS_TEXT_SPACE)
                     << this->LoadString(IDS_TEXT_OF)
                  ...
                  if (update_time != prev_update_time) {
                    ...
                    ss << this->LoadString(IDS_TEXT_PAREN_OPEN);
                    ss << nformat_ui(
                        nwritten_since_update * 1000U /
                        ((update_time - prev_update_time) * 1ULL << 20));
                    ss << this->LoadString(IDS_TEXT_SPACE)
                       << this->LoadString(IDS_TEXT_MIB_S);
                  ...
                  ss << _T('\n');
                  ss.append(line_buffer.data() +
                                static_cast<ptrdiff_t>(begin_offset),
                            line_buffer.size() - begin_offset);
                  std::tstring const &text = ss.str();
                  dlg.SetProgressText(text);
                  dlg.SetProgress(
                      static_cast<long long>(i),
                      static_cast<long long>(locked_indices.size()));
                  dlg.Flush();
                }
              }
              // NOTE: We assume there are no double-quotes here, so we don't
              // handle escaping for that! This is a valid assumption for this
              // program.
              if (tabsep) {
                bool may_contain_tabs = false;
                if (may_contain_tabs &&
                    line_buffer.find(_T('\t'), begin_offset) !=
                        std::tstring::npos) {
                  line_buffer.insert(line_buffer.begin() +
                                         static_cast<ptrdiff_t>(begin_offset),
                                     _T('\"'));
                  line_buffer.push_back(_T('\"'));
                }
              } else {
                if (line_buffer.find(_T(','), begin_offset) !=
                        std::tstring::npos ||
                    line_buffer.find(_T('\''), begin_offset) !=
                        std::tstring::npos) {
                  line_buffer.insert(line_buffer.begin() +
            ...
            if (shell_file_list && is_ads) {
              if (!warned_about_ads) {
                ...
                (dlg.IsWindow() ? ATL::CWindow(dlg.GetHWND()) : *this)
                    .MessageBox(this->LoadString(IDS_COPYING_ADS_PROBLEM_BODY),
                                this->LoadString(IDS_WARNING_TITLE),
              ...
              line_buffer.erase(line_buffer.begin() +
                                    static_cast<ptrdiff_t>(entry_begin_offset),
                                line_buffer.end());
            }
            ...
            if (should_flush) {
              if (output) {
                ...
                using std::max;
                line_buffer_utf8.resize(
                    max(line_buffer_utf8.size(), (line_buffer.size() + 1) * 6),
                    _T('\0'));
                int const cch = WideCharToMultiByte(
                    CP_UTF8, 0, line_buffer.empty() ? NULL : &line_buffer[0],
                    static_cast<int>(line_buffer.size()), &line_buffer_utf8[0],
                    static_cast<int>(line_buffer_utf8.size()), NULL, NULL);
                if (cch > 0) {
                  nwritten_since_update +=
                      _write(output, line_buffer_utf8.data(),
                             sizeof(*line_buffer_utf8.data()) *
                                 static_cast<size_t>(cch));
                }
#else
                nwritten_since_update +=
                    _write(output, line_buffer.data(),
                           sizeof(*line_buffer.data()) * line_buffer.size());
#endif
                line_buffer.clear();
              } else {
                nwritten_since_update =
                    line_buffer.size() * sizeof(*line_buffer.begin());
              }
            }
          }
          if (clipboard) {
            unsigned int const format =
                shell_file_list                              ? CF_HDROP
                : sizeof(*line_buffer.data()) > sizeof(char) ? CF_UNICODETEXT
                                                             : CF_TEXT;
            if (HGLOBAL const resized = GlobalReAlloc(
                    GlobalHandle(line_buffer.c_str()),
                    (line_buffer.size() + 1) * sizeof(*line_buffer.data()),
                    0)) {
              HANDLE const result = SetClipboardData(format, resized);
              // Clear the string, because we put it in an invalid state -- the
              // underlying buffer is too small now!
  ...
  void DoubleClick(int index) {
    Wow64Disable const wow64disable(true);
    NtfsIndex const volatile *const i =
        this->results.item_index(static_cast<size_t>(index));
    std::tvstring path;
    path = i->root_path(),
    lock(i)->get_path(this->results[static_cast<size_t>(index)].key(), path,
                      false);
    remove_path_stream_and_trailing_sep(path);
    std::auto_ptr<
        std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
        ...
        p(new std::pair<std::pair<CShellItemIDList, ATL::CComPtr<IShellFolder>>,
                        std::vector<CShellItemIDList>>());
    SFGAOF sfgao = 0;
    std::tstring const path_directory(path.begin(),
                                      dirname(path.begin(), path.end()));
    HRESULT hr = SHParseDisplayName(path_directory.c_str(), NULL,
                                    &p->first.first, 0, &sfgao);
    if (hr == S_OK) {
      ATL::CComPtr<IShellFolder> desktop;
      hr = SHGetDesktopFolder(&desktop);
      if (hr == S_OK) {
    ...
    if (hr == S_OK && basename(path.begin(), path.end()) != path.end()) {
      p->second.resize(1);
      hr = SHParseDisplayName((path.c_str(), path.empty() ? NULL : &path[0]),
                              NULL, &p->second.back().m_pidl, sfgao, &sfgao);
    }
    SHELLEXECUTEINFO shei = {sizeof(shei),
                             SEE_MASK_INVOKEIDLIST | SEE_MASK_UNICODE,
                             *this,
                             NULL,
                             NULL,
                             p->second.empty() ? path_directory.c_str() : NULL,
                             path_directory.c_str(),
                             SW_SHOWDEFAULT,
                             0,
                             p->second.empty() ? NULL
                                               : p->second.back().m_pidl};
    ShellExecuteEx(&shei);
  }
  ...
  void OnFileNameChange(UINT /*uNotifyCode*/, int /*nID*/, HWND /*hWnd*/) {
    if (!this->autocomplete_called) {
      if (SHAutoComplete(this->txtPattern, SHACF_FILESYS_ONLY | SHACF_USETAB) ==
          S_OK) // Needs CoInitialize() to have been called (use CoInit)
      {
  ...
  LRESULT OnFileNameArrowKey(LPNMHDR pnmh) {
    ...
    if (p->vkey == VK_UP || p->vkey == VK_DOWN) {
      this->cmbDrive.SendMessage(
          p->hdr.code == CSearchPattern::CUN_KEYDOWN ? WM_KEYDOWN : WM_KEYUP,
          p->vkey, p->lParam);
    } else {
  ...
  size_t get_file_type_blocking(
      std::tvstring &name, size_t const name_offset,
      unsigned int const file_attributes) const // TODO: should be 'volatile'
  {
    size_t result = 0;
    while (name.size() > name_offset && hasdirsep(name)) {
      name.erase(name.end() - 1);
    }
    atomic_namespace::unique_lock<atomic_namespace::recursive_mutex> const lock(
        this->type_cache_mutex);
    this->type_cache_str.assign(
        name.begin() + static_cast<ptrdiff_t>(name_offset),
        basename(name.begin() + static_cast<ptrdiff_t>(name_offset),
                 name.end()));
    size_t ext_offset, ext_length;
    if (file_attributes & FILE_ATTRIBUTE_DIRECTORY) {
      ext_offset =
          name.size() - (!name.empty() && *(name.end() - 1) == _T('.'));
      ext_length = name.size() - static_cast<ptrdiff_t>(ext_offset);
    } else {
      // TODO: Name should not include alternate data stream...
      size_t const name_end = static_cast<size_t>(
          std::find(basename(name.begin(), name.end()), name.end(), _T(':')) -
          name.begin());
      ext_offset = fileext(name.begin() + static_cast<ptrdiff_t>(name_offset),
                           name.begin() + static_cast<ptrdiff_t>(name_end)) -
                   name.begin();
    ...
    this->type_cache_str.append(
        name.data() + static_cast<ptrdiff_t>(ext_offset), ext_length);
    if (!this->type_cache_str.empty() &&
        *(this->type_cache_str.end() - 1) == _T('.') &&
        (this->type_cache_str.size() <= 1 ||
         isdirsep(*(this->type_cache_str.end() - 2)))) {
      this->type_cache_str.erase(this->type_cache_str.end() - 1,
                                 this->type_cache_str.end());
    }
    name.erase(name.begin() + static_cast<ptrdiff_t>(name_offset), name.end());
    if (!this->type_cache_str.empty()) {
      unsigned int const usable_attributes =
          file_attributes &
          (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_SYSTEM |
           FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_DEVICE |
           FILE_ATTRIBUTE_REPARSE_POINT) /* restrict this to prevent redundant
                                            storage */
          ;
      ...
      this->type_cache_str.insert(
          this->type_cache_str.end(),
          reinterpret_cast<TCHAR const *>(&usable_attributes),
          sizeof(usable_attributes) / sizeof(TCHAR));
      std::transform(this->type_cache_str.begin(),
                     this->type_cache_str.begin() +
                         static_cast<ptrdiff_t>(type_cache_str_length),
                     this->type_cache_str.begin(),
                     char_transformer<TCHAR, totupper<TCHAR>>());
      if (usable_attributes & FILE_ATTRIBUTE_DIRECTORY) {
        adddirsep(this->type_cache_str);
      }
      TypeInfoCache::value_type::second_type &found =
          this->type_cache[this->type_cache_str];
      if (found.empty() && !this->type_cache_str.empty()) {
        SHFILEINFO shfi;
        Wow64Disable const wow64disable(true);
        if (SHGetFileInfo(this->type_cache_str.c_str(), usable_attributes,
                          &shfi, sizeof(shfi),
  ...
  void GetSubItemText(size_t const j, int const subitem, bool const for_ui,
                      std::tvstring &text, bool const lock_index = true) const {
    ...
    case COLUMN_INDEX_PATH:
      ...
      break;
    case COLUMN_INDEX_TYPE: {
      size_t const k = text.size();
      text += i->root_path();
      unsigned int attributes = 0;
      i->get_path(key, text, true, &attributes);
      this->get_file_type_blocking(text, k, attributes);
    } break;
    case COLUMN_INDEX_SIZE:
  ...
  LRESULT OnFilesIncrementalSearch(LPNMHDR pnmh) {
    ...
    if (pLV->lvfi.flags & (LVFI_STRING | LVFI_PARTIAL)) {
      ...
      string_matcher matcher(string_matcher::pattern_verbatim,
                             string_matcher::pattern_option_case_insensitive,
      ...
      for (int i = 0; i < n; ++i) {
        int const iItem =
            (pLV->lvfi.lParam + (needle_length > 1 ? 0 : 1) + i) % n;
        if (!(pLV->lvfi.flags & LVFI_WRAP) && iItem == 0 && i != 0) {
          break;
        }
        locked_results(static_cast<size_t>(iItem));
        text.clear();
        this->GetSubItemText(static_cast<size_t>(iItem), COLUMN_INDEX_NAME,
                             true, text, false);
        bool const match =
            (pLV->lvfi.flags & (LVFI_PARTIAL | (0x0004 /*LVFI_SUBSTRING*/)))
                ? text.size() >= needle_length &&
                      matcher.is_match(text.data(), needle_length)
                : text.size() == needle_length &&
                      matcher.is_match(text.data(), needle_length);
        if (match) {
  ...
  void get_subitem_text(size_t const item, int const subitem,
                        std::tvstring &text) {
    text.clear();
    ...
    if ((this->lvFiles.GetStyle() & LVS_OWNERDATA) != 0 &&
      ...
      if (pLV->item.iSubItem == 0) {
        ...
        if (this->lvFiles.GetItemRect(pLV->item.iItem, &rcitem, LVIR_ICON) &&
          ...
          int iImage =
              this->CacheIcon(path, static_cast<int>(pLV->item.iItem),
                              lock(this->results.item_index(j))
                                  ->get_stdinfo(this->results[j].key().frs())
                                  .attributes(),
                              GetMessageTime());
          if (iImage >= 0) {
  ...
  LRESULT OnFilesListCustomDraw(LPNMHDR pnmh) {
    ...
    COLORREF sparseColor =
        RGB(GetRValue(compressedColor),
    ...
    if (pLV->nmcd.dwItemSpec < this->results.size()) {
      ...
      unsigned long const attrs =
          lock(this->results.item_index(iresult))
              ->get_stdinfo(this->results[iresult].key().frs())
              .attributes();
      ...
      case CDDS_ITEMPREPAINT | CDDS_SUBITEM:
        if (pLV->nmcd.dwItemSpec % 2 && false) {
          unsigned char const v = 0xF8;
          pLV->clrTextBk = RGB(v, v, v);
        }
        if ((this->small_image_list() == this->imgListLarge ||
             this->small_image_list() == this->imgListExtraLarge) &&
            pLV->iSubItem == 1) {
      ...
      case CDDS_ITEMPOSTPAINT | CDDS_SUBITEM:
        result = CDRF_SKIPDEFAULT;
        {
          ...
          this->GetSubItemText(static_cast<size_t>(pLV->nmcd.dwItemSpec),
                               pLV->iSubItem, true, itemText);
          WTL::CDCHandle dc(pLV->nmcd.hdc);
          RECT rcTwips = pLV->nmcd.rc;
          rcTwips.left =
              (int)((rcTwips.left + 6) * 1440 / dc.GetDeviceCaps(LOGPIXELSX));
          rcTwips.right =
              (int)(rcTwips.right * 1440 / dc.GetDeviceCaps(LOGPIXELSX));
          rcTwips.top =
              (int)(rcTwips.top * 1440 / dc.GetDeviceCaps(LOGPIXELSY));
          rcTwips.bottom =
              (int)(rcTwips.bottom * 1440 / dc.GetDeviceCaps(LOGPIXELSY));
          int const savedDC = dc.SaveDC();
          {
            std::replace(itemText.begin(), itemText.end(), _T(' '),
                         _T('\u00A0'));
            {
              ...
              itemText.erase(itemText.begin(),
                             itemText.begin() +
                                 static_cast<ptrdiff_t>(prev_size));
            }
            if (!this->richEdit) {
              this->hRichEdit = LoadLibrary(_T("riched20.dll"));
              this->richEdit.Create(this->lvFiles, NULL, 0, ES_MULTILINE,
                                    WS_EX_TRANSPARENT);
            ...
            if ((attrs & 0x40000000) != 0) {
              ...
              bool const selected =
                  (this->lvFiles.GetItemState(
                       static_cast<int>(pLV->nmcd.dwItemSpec), LVIS_SELECTED) &
                   LVIS_SELECTED) != 0;
              format.crTextColor = GetSysColor(
                  selected && this->lvFiles.IsThemeNull() ? COLOR_HIGHLIGHTTEXT
                                                          : COLOR_WINDOWTEXT);
            }
            ...
            formatRange.rc.top +=
                (formatRange.rc.bottom - formatRange.rc.top - height) / 2;
  ...
  void OnClose(UINT /*uNotifyCode*/ = 0, int nID = IDCANCEL,
               HWND /*hWnd*/ = NULL) {
    this->DestroyWindow();
    PostQuitMessage(nID);
    // this->EndDialog(nID);
  ...
  LRESULT OnDeviceChange(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam) {
    ...
    case DBT_DEVICEQUERYREMOVE: {
      DEV_BROADCAST_HDR const &header =
  ...
  LRESULT OnReady(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam) {
    this->Refresh(true);
    RegisterWaitForSingleObject(&this->hWait, hEvent, &WaitCallback,
                                this->m_hWnd, INFINITE, WT_EXECUTEINUITHREAD);
    return 0;
  }
  ...
  BOOL CheckAndCreateIcon(bool checkVisible) {
    NOTIFYICONDATA nid = {sizeof(nid),
                          *this,
                          ...
                          WM_NOTIFYICON,
                          this->GetIcon(FALSE)};
    _tcsncpy(nid.szTip, this->LoadString(IDS_APPNAME),
             sizeof(nid.szTip) / sizeof(*nid.szTip));
    return (!checkVisible || (this->ShouldWaitForWindowVisibleOnStartup() &&
                              !this->IsWindowVisible())) &&
           Shell_NotifyIcon(NIM_ADD, &nid);
  }

  LRESULT OnTaskbarCreated(UINT /*uMsg*/, WPARAM /*wParam*/,
                           LPARAM /*lParam*/) {
  ...
  void OnHelp(UINT /*uNotifyCode*/, int nID, CWindow /*wndCtl*/) {
    ...
    switch (nID) {
    case ID_HELP_DONATE:
      body.Format(this->LoadString(IDS_HELP_DONATE_BODY),
                  this->get_project_url(IDS_PROJECT_USER_FRIENDLY_URL).c_str());
      title = this->LoadString(IDS_HELP_DONATE_TITLE);
      type = ((type & ~static_cast<unsigned int>(MB_OK)) | MB_OKCANCEL);
      break;
    case ID_HELP_TRANSLATION:
      body.Format(this->LoadString(IDS_HELP_TRANSLATION_BODY),
                  static_cast<LPCTSTR>(get_ui_locale_name()));
    ...
    if (!body.IsEmpty() || !title.IsEmpty()) {
      int const r = this->MessageBox(static_cast<LPCTSTR>(body),
                                     static_cast<LPCTSTR>(title), type);
      switch (nID) {
      case ID_HELP_DONATE:
      case ID_HELP_TRANSLATION:
        if (r == IDOK) {
  ...
  void OnHelpBugs(UINT /*uNotifyCode*/, int /*nID*/, CWindow /*wndCtl*/) {
    ...
    body.Format(this->LoadString(IDS_TEXT_REPORT_ISSUES),
                this->get_project_url(IDS_PROJECT_USER_FRIENDLY_URL).c_str());
    {
      body += _T("\x2022");
    ...
    if (this->MessageBox(body, this->LoadString(IDS_HELP_BUGS_TITLE),
  ...
  RefCountedCString get_project_url(unsigned short const id) {
    ...
    result.Format(this->LoadString(id),
                  this->LoadString(IDS_PROJECT_NAME).c_str());
    return result;
  }

  void open_project_page() {
    ShellExecute(NULL, _T("open"),
                 this->get_project_url(IDS_PROJECT_ROBUST_URL), NULL, NULL,
                 SW_SHOWNORMAL);
  }
  ...
  void OnViewLargeIcons(UINT /*uNotifyCode*/, int /*nID*/, CWindow /*wndCtl*/) {
    ...
    this->GetMenu().CheckMenuItem(ID_VIEW_LARGEICONS,
                                  large ? MF_CHECKED : MF_UNCHECKED);
  }

  void OnViewGridlines(UINT /*uNotifyCode*/, int /*nID*/, CWindow /*wndCtl*/) {
    this->lvFiles.SetExtendedListViewStyle(
        this->lvFiles.GetExtendedListViewStyle() ^ LVS_EX_GRIDLINES);
    this->lvFiles.RedrawWindow();
    this->GetMenu().CheckMenuItem(
        ID_VIEW_GRIDLINES,
        (this->lvFiles.GetExtendedListViewStyle() & LVS_EX_GRIDLINES)
            ? MF_CHECKED
            : MF_UNCHECKED);
  ...
  void OnViewFitColumns(UINT /*uNotifyCode*/, int nID, CWindow /*wndCtl*/) {
    ...
    if (nID == ID_VIEW_FITCOLUMNSTOWINDOW) {
      ...
      int const client_width =
          (std::max)(1,
                     (int)(rect.right - rect.left) -
                         ((this->lvFiles.GetWindowLong(GWL_STYLE) & WS_VSCROLL)
                              ? 0
      ...
      struct TextGetter : WTL::CListViewCtrl {
        ...
        static ptrdiff_t invoke(void *const me, int const item,
                                int const subitem,
                                ListViewAdapter::String &result) {
          TextGetter *const this_ =
              static_cast<TextGetter *>(static_cast<WTL::CListViewCtrl *>(me));
          this_->me->get_subitem_text(item, subitem, result);
          return static_cast<ptrdiff_t>(result.size());
        }
      } instance(*this);
      autosize_columns(ListViewAdapter(
          instance, this->cached_column_header_widths, &TextGetter::invoke));
    }
  }

  void OnOptionsFastPathFiltering(UINT /*uNotifyCode*/, int nID,
                                  CWindow /*wndCtl*/) {
    this->trie_filtering = !this->trie_filtering;
    this->GetMenu().CheckMenuItem(ID_OPTIONS_FAST_PATH_SEARCH,
                                  this->trie_filtering ? MF_CHECKED
                                                       : MF_UNCHECKED);
  }
  ...
  void Refresh(bool const initial) {
    if (!initial && this->indices_created < this->cmbDrive.GetCount() - 1) {
      // Ignore the request due to potential race condition... at least wait
      // until all the threads have started! Otherwise, if the user presses F5
      // we will delete some entries, which will invalidate threads' copies of
      // their index in the combobox
      return;
    }
    ...
    for (int ii = 0; ii < this->cmbDrive.GetCount(); ++ii) {
      if (selected == 0 || ii == selected) {
        ...
        if (q || (initial && ii != 0)) {
          ...
          struct OverlappedNtfsMftReadPayloadDerived
              : OverlappedNtfsMftReadPayload {
            HWND m_hWnd;
            explicit OverlappedNtfsMftReadPayloadDerived(
                IoCompletionPort volatile &iocp,
                intrusive_ptr<NtfsIndex volatile> p, HWND const &m_hWnd,
                Handle const &closing_event)
                : OverlappedNtfsMftReadPayload(iocp, p, closing_event),
                  m_hWnd(m_hWnd) {}
            void preopen() override {
              this->OverlappedNtfsMftReadPayload::preopen();
              if (this->m_hWnd) {
                DEV_BROADCAST_HANDLE dev = {
                    sizeof(dev), DBT_DEVTYP_HANDLE, 0, p->volume(),
                    reinterpret_cast<HDEVNOTIFY>(this->m_hWnd)};
                dev.dbch_hdevnotify = RegisterDeviceNotification(
                    this->m_hWnd, &dev, DEVICE_NOTIFY_WINDOW_HANDLE);
              }
            }
          };
          intrusive_ptr<OverlappedNtfsMftReadPayload> p(
              new OverlappedNtfsMftReadPayloadDerived(
                  this->iocp, q, this->m_hWnd, this->closing_event));
...
#pragma warning(suppress : 4555)
  ...
  MESSAGE_HANDLER_EX(WM_DEVICECHANGE,
                     OnDeviceChange) // Don't use MSG_WM_DEVICECHANGE(); it's
                                     // broken (uses DWORD)
  ...
  NOTIFY_HANDLER_EX(IDC_LISTFILES, LVN_INCREMENTALSEARCH,
                    OnFilesIncrementalSearch)
  ...
  NOTIFY_HANDLER_EX(IDC_EDITFILENAME, CSearchPattern::CUN_KEYDOWN,
                    OnFileNameArrowKey)
...
inline void My_Stable_sort_unchecked1(
    _BidIt _First, _BidIt _Last, _Diff _Count,
    std::_Temp_iterator<_Ty> &_Tempbuf,
    _Pr &_Pred) { // sort preserving order of equivalents, using _Pred
  {               // sort halves and merge
    ...
    if (_Count2 <=
        _Tempbuf
            ._Maxlen()) { // temp buffer big enough, sort each half using buffer
      std::_Buffered_merge_sort_unchecked(_First, _Mid, _Count2, _Tempbuf,
                                          _Pred);
    ...
    std::_Buffered_merge_unchecked(_First, _Mid, _Last, _Count2,
                                   _Count - _Count2, _Tempbuf,
                                   _Pred); // merge halves
  }
}
#endif
...
RefCountedCString get_app_guid() {
  ...
  guid_str.Insert(guid_str.GetLength(),
                  _T("{CB77990E-A78F-44dc-B382-089B01207F02}"));
  return guid_str;
}

std::pair<int, std::tstring>
extract_and_run_if_needed(HINSTANCE hInstance, int argc, TCHAR *const argv[]) {
  int result = -1;
  ...
  if (argc > 0) {
    ...
    module_path.resize(GetModuleFileName(
        hInstance, module_path.empty() ? NULL : &*module_path.begin(),
        static_cast<unsigned int>(module_path.size())));
  }
  if (Wow64::is_wow64() &&
      !string_matcher(string_matcher::pattern_regex,
                      string_matcher::pattern_option_case_insensitive,
    ...
    if (!IsDebuggerPresent()) {
      HRSRC hRsrs = NULL;
      WORD langs[] = {GetUserDefaultUILanguage(),
                      MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL),
                      MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                      MAKELANGID(LANG_NEUTRAL, SUBLANG_SYS_DEFAULT),
                      MAKELANGID(LANG_INVARIANT, SUBLANG_NEUTRAL)};
      for (size_t ilang = 0; ilang < sizeof(langs) / sizeof(*langs) && !hRsrs;
           ++ilang) {
      ...
      if (pBinary) {
        ...
        tempDir.resize(
            GetTempPath(static_cast<DWORD>(tempDir.size()), &tempDir[0]));
        if (!tempDir.empty()) {
          ...
          for (int pass = 0; !success && pass < 2; ++pass) {
            ...
            std::filebuf file;
            std::ios_base::openmode const openmode = std::ios_base::out |
                                                     std::ios_base::trunc |
                                                     std::ios_base::binary;
            ...
            if (success) {
              ...
              file.sputn(
                  static_cast<char const *>(pBinary),
          ...
          if (success) {
            ...
            if (hJob != NULL &&
              ...
              if (CreateProcess(
                      fileName.c_str(), GetCommandLine(), NULL, NULL, FALSE,
...
int _tmain(int argc, TCHAR *argv[]) {
  std::pair<int, std::tstring> extraction_result =
      extract_and_run_if_needed(GetModuleHandle(NULL), argc, argv);
  ...
  if (argc <= 1) {
    ...
    int const stderr_fd = _fileno(stderr), stdout_fd = _fileno(stdout);
    (void)stderr_fd;
    (void)stdout_fd;
    result = 0;
    try {
      ...
      std::vector<intrusive_ptr<NtfsIndex>> indices;
      {
        ...
        if (argc > 2 && _tcscmp(argv[2], _T("*")) != 0) {
          ...
          for (size_t i = 0;; ++i) {
            ...
            std::tvstring path_name(arg.data() + static_cast<ptrdiff_t>(i),
                                    (~j ? j : arg.size()) - i);
            adddirsep(path_name);
        ...
        for (size_t i = 0; i != path_names.size(); ++i) {
          if (matchop.prematch(path_names[i])) {
            indices.push_back(static_cast<intrusive_ptr<NtfsIndex>>(
                new NtfsIndex(path_names[i])));
          }
        }
      }
      ...
      if (argc > 3) {
        if (_tcsicmp(argv[3], _T("")) == 0) {
          ...
          throw std::invalid_argument(
              "invalid column list; currently only \"\" (default) and \"*\" "
      ...
      for (size_t i = 0; i != indices.size(); ++i) {
        if (void *const volume = indices[i]->volume()) {
          IoPriority(reinterpret_cast<uintptr_t>(volume), winnt::IoPriorityLow)
              .swap(set_priorities[i]);
        }
      ...
      while (!pending.empty()) {
        ...
        {
          IoPriority const raise_first_priority(
              *pending.begin(), set_priorities[*pending.begin()].old());
          for (nwait_handles = 0;
               nwait_handles != pending.size() &&
               nwait_handles != sizeof(wait_handles) / sizeof(*wait_handles);
               ++nwait_handles) {
            wait_handles[nwait_handles] = reinterpret_cast<HANDLE>(
                indices[pending[nwait_handles]]->finished_event());
          }
          DWORD const wait_result = WaitForMultipleObjects(
              nwait_handles, wait_handles, FALSE, INFINITE);
          ...
          if (wait_result < WAIT_OBJECT_0 + nwait_handles) {
            ...
            pending.erase(pending.begin() +
                          static_cast<ptrdiff_t>(wait_result - WAIT_OBJECT_0));
          }
        }
        if (i) {
          ...
          class Writer {
            ...
            unsigned long long
                second_fraction_denominator /* NOTE: Too large a value is
                                               useless with clock(); use a more
                                               precise timer if needed */
                ;
            clock_t tprev;
          ...
          public:
            explicit Writer(int const output, size_t const max_buffer_size = 0)
                : output(reinterpret_cast<HANDLE>(_get_osfhandle(output))),
                  output_isatty(!!isatty(output)),
                  output_isdevnull(isdevnull(output)),
                  max_buffer_size(max_buffer_size),
                  second_fraction_denominator(1 << 11), tprev(clock()),
                  nwritten_since_update() {}
            void operator()(std::tvstring &line_buffer,
                            bool const force = false) {
              if (!output_isdevnull) {
                clock_t const tnow = clock();
                unsigned long nwritten;
                if (force || line_buffer.size() >= max_buffer_size ||
                    (tnow - tprev) * 1000 * second_fraction_denominator >=
                        CLOCKS_PER_SEC) {
                  if ((!output_isatty ||
                       (!WriteConsole(
                            output, line_buffer.data(),
                            static_cast<unsigned int>(line_buffer.size()),
                            &nwritten, NULL) &&
                        GetLastError() == ERROR_INVALID_HANDLE))) {
#if defined(_UNICODE) && _UNICODE
                    using std::max;
                    line_buffer_utf8.resize(max(line_buffer_utf8.size(),
                                                (line_buffer.size() + 1) * 6),
                                            _T('\0'));
                    int const cch = WideCharToMultiByte(
                        CP_UTF8, 0,
                        line_buffer.empty() ? NULL : &line_buffer[0],
                        static_cast<int>(line_buffer.size()),
                        &line_buffer_utf8[0],
                        static_cast<int>(line_buffer_utf8.size()), NULL, NULL);
                    if (cch > 0) {
                      nwritten_since_update +=
                          WriteFile(output, line_buffer_utf8.data(),
                                    sizeof(*line_buffer_utf8.data()) *
                                        static_cast<size_t>(cch),
                                    &nwritten, NULL);
                    }
#else
                    nwritten_since_update += WriteFile(
                        output, line_buffer.data(),
                        sizeof(*line_buffer.data()) * line_buffer.size(),
                        &nwritten, NULL);
#endif
          ...
          i->matches(
              [&](TCHAR const *const name, size_t const name_length,
                  bool const ascii, NtfsIndex::key_type const &key,
                  size_t const depth)
               ...
               */
              {
                size_t high_water_mark = 0,
                       *phigh_water_mark =
                           matchop.is_path_pattern ? &high_water_mark : NULL;
                TCHAR const *const path_begin = name;
                bool const match =
                    ascii ? matchop.matcher.is_match(
                                static_cast<char const *>(
                ...
                if (match) {
                  ...
                  if (output_everything) {
                    ...
                    NtfsIndex::standard_info const &stdinfo =
                        i->get_stdinfo(key.frs());
                    /*  File attribute abbreviations:
                     * https://en.wikipedia.org/wiki/File_attribute#Types */
                    line_buffer.push_back(_T('\t'));
                    CMainDlg::SystemTimeToStringImpl(stdinfo.created,
                                                     line_buffer, true, true,
                                                     time_zone_bias, lcid);
                    line_buffer.push_back(_T('\t'));
                    CMainDlg::SystemTimeToStringImpl(stdinfo.written,
                                                     line_buffer, true, true,
                                                     time_zone_bias, lcid);
                    line_buffer.push_back(_T('\t'));
                    CMainDlg::SystemTimeToStringImpl(stdinfo.accessed,
                                                     line_buffer, true, true,
                                                     time_zone_bias, lcid);
                    line_buffer.push_back(_T('\t'));
                    line_buffer +=
                        nformat(static_cast<unsigned int>(sizeinfo.treesize));
                  }
                  line_buffer.push_back(_T('\n'));
                  flush_if_needed(line_buffer);
                }
                return match || !(matchop.is_path_pattern && phigh_water_mark &&
                                  *phigh_water_mark < name_length);
              },
              current_path, matchop.is_path_pattern, matchop.is_stream_pattern,
              match_attributes);
...
int __stdcall _tWinMain(HINSTANCE const hInstance, HINSTANCE /*hPrevInstance*/,
  ...
  std::pair<int, std::tstring> extraction_result =
      extract_and_run_if_needed(hInstance, argc, argv);
  if (extraction_result.first != -1) {
    return extraction_result.first;
  }
  ...
  (void)argc;
  (void)argv;
  if (get_subsystem(&__ImageBase) != IMAGE_SUBSYSTEM_WINDOWS_GUI) {
  ...
  if (GetLocaleInfo(LOCALE_USER_DEFAULT,
                    0x00000070 /*LOCALE_IREADINGLAYOUT*/ | LOCALE_RETURN_NUMBER,
    ...
    right_to_left = !!(
        GetWindowLongPtr(FindWindow(_T("Shell_TrayWnd"), NULL), GWL_EXSTYLE) &
        WS_EX_LAYOUTRTL);
  }
  if (right_to_left) {
  ...
  __if_exists(_Module) { _Module.Init(NULL, hInstance); }
  {
    ...
    HANDLE hEvent = CreateEvent(
        NULL, FALSE, FALSE, _T("Local\\") + appname + _T(".") + get_app_guid());
    if (hEvent != NULL && GetLastError() != ERROR_ALREADY_EXISTS) {
      ...
      if (!module_path.empty()) {
        ...
        module_name.erase(module_name.begin(),
                          basename(module_name.begin(), module_name.end()));
        std::tstring module_directory = module_path;
        module_directory.erase(
            dirname(module_directory.begin(), module_directory.end()),
            module_directory.end());
        if (!module_directory.empty()) {
          module_directory += getdirsep();
        }
        std::tstring const period = TEXT(".");
        std::tstring const mui_ext = TEXT("mui");
        std::tstring const locale_name =
            static_cast<LPCTSTR>(get_ui_locale_name());
        std::tstring const mui_paths[] = {
            module_directory + locale_name + getdirsep() + module_name +
                period + mui_ext,
            module_directory + module_name + period + locale_name + period +
                mui_ext,
            module_directory + locale_name + getdirsep() + module_name +
                period + locale_name + period + mui_ext,
            module_directory + module_name + period + mui_ext,
        };
        for (size_t i = 0;
             i != sizeof(mui_paths) / sizeof(*mui_paths) && !mui_module; ++i) {
          unsigned int const mui_load_flags = LOAD_LIBRARY_AS_DATAFILE;
          for (int pass = 0; pass < 2 && !mui_module; ++pass) {
            mui_module = LoadLibraryEx(
                mui_paths[i].c_str(), NULL,
                mui_load_flags |
                    (!pass ? 0x00000020 /*LOAD_LIBRARY_AS_IMAGE_RESOURCE only
      ...
      PulseEvent(hEvent); // PulseThread() is normally unreliable, but we don't
                          // really care here...
      result = GetLastError();
    }
  }
